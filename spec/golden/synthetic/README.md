...
| `gradient-horizontal.png` | 텍스처 샘플링·보간 + **좌우 방향** | 좌 `(255,32,32)` 우 `(32,32,255)` · 같은 행 안에서만 변한다(세로 변화 0) |
...
**[2026-08-19] `gradient-horizontal` 추가 — 종전 넷은 좌우 미러링을 못 잡았다.**
단색 셋은 대칭이고 `gradient-vertical` 은 x 에 불변이라, `uv.x` 부호가 뒤집히거나
정점 winding 이 반대가 되어도 **픽셀이 그대로**였다. 새 케이스는 좌 빨강 / 우 파랑이라
그 회귀에서 즉시 갈린다. 헬퍼(`TestSupport.swift:89 horizontalGradientTex`)는 이미 있었고
다른 테스트가 쓰고 있었다 — 골든 세트만 안 쓰고 있었다.

기준선 검증은 눈이 아니라 **수치**로 했다(부트스트랩 아티팩트를 받아 디코드):
세로 방향 최대 변화 0 · 가로 방향 223 · 좌 `(255,32,32)` · 우 `(32,32,255)` ·
R 단조감소 · B 단조증가. 대조군 `gradient-vertical` 은 정확히 반대(세로 223 / 가로 0).
같은 부트스트랩에서 기존 넷은 **바이트 동일**하게 재생됐다 — 캡처가 결정적이라는 뜻이다.
