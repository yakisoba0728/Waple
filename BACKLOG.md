...
| [하네스](#하네스) | 게이트 오탐·소요가 거슬릴 때 | ~~F402/F403 골든 미커밋~~(해소 — `c8734e5b`·`f494c1b5`, `spec/golden/snapshot/baseline-618d16f`) · F398 벽시계 결합 33사이트 · F144 임계 고정 · **3538758087 비디오-백드 플레이크** · **🔴 3706286085 비결정으로 재베이스라인 차단**(아래) |
...
- **🟠 골든 재베이스라인 — `3706286085` 잔여 비결정**(2026-08-16, **원인 규명 완료 · 채택 결정 대기**)
  ...
  **제 변경 탓이 아님을 A/B 로 확정했다**: 같은 씬을 `33641ae`(그 4커밋 이전) 빌드로 3회 캡처하면
  ...
  이미 `spec/golden/nondeterminism.json` 의 불안정 29종 안이다. `baseline-618d16f` 를 뜰 때
  ...
  **게이트는 확률 게임이다 — 2026-08-16 밤 `00a9534` 에서 `rebaseline-golden.sh` 를 완주시키니
  ...
  현행 `baseline-618d16f` 대비 **원해시 상이 87/170**(비디오-백드 24종 포함)이었다.
