...
**현재 상태** (2026-08-16): 유지보수 모드. 테스트 2,159개 · CI 그린(`macos-26`).
...
- **🔴 골든 재베이스라인이 막혀 있다 — `3706286085` 프로세스 간 비결정**(2026-08-16, **차단 중**)
  ...
  **제 변경 탓이 아님을 A/B 로 확정했다**: 같은 씬을 `33641ae`(그 4커밋 이전) 빌드로 3회 캡처하면
  ...
  성질: `meanLuma` 는 소수 6자리까지 동일하고 `maxdiff 3` / 약 12픽셀 — 미세 지터다. 후보는
  섀도우 아틀라스 할당 순서나 딕셔너리 순회 순서 같은 프로세스 간 비결정이며 **미규명**이다.

  **다음 사람에게**: 게이트의 전부-비트동일 규약을 약화시켜 통과시키지 마라. 그건 이 리포가
  반복해서 당한 실패형이다. 지터의 원인을 좁힌 뒤 재베이스라인하는 것이 순서다. 그때까지
  `golden-gate.sh` 의 FAIL 52종은 "낡은 골든" 으로 읽어야 한다(회귀가 아니다).
