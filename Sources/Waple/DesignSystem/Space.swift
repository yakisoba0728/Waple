...
///
/// **왜 스케일인가.** 종전 토큰은 `Metrics.gap = 8` 하나뿐이었고, 나머지 간격은 화면마다
/// 리터럴로 흩어져 있었다 — 실측하면 `2·3·4·6·8·10·12·14·16·20·22·24` 열두 종이다. 그중
/// `3`(SelectionPanelView 메타 스택)과 `4`(OnboardingView 제목 스택)처럼 **같은 역할인데
/// 1pt 다른** 쌍이 여럿이라, 화면을 나란히 놓으면 정렬이 미세하게 어긋난다. 눈으로는
/// "왠지 정돈이 덜 됐다" 로만 보이고 원인을 못 찾는 종류의 어긋남이다.
///
