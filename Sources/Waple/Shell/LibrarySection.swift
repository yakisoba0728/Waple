...
/// ## 왜 선택을 저장하지 않고 유도하는가
///
/// 선택은 저장하지 않고 **상태에서 매번 유도**한다(`selection(for:)`). 사이드바로 표현할 수
/// 없는 조합(예: 씬+웹 동시 선택)이면 `nil` 을 돌려 아무 행도 강조되지 않게 한다 — 틀린 행을
/// 강조하는 것보다 낫다.
///
/// **[정정 r4-24] 종전 근거였던 "쓰기 주체가 둘" 은 지금 거짓이다.** 그 문단은 "툴바 필터
/// (종전 `FilterSidebarView`)도 같은 `criteria.types`·`favoritesOnly` 를 쓴다" 를 전제로
/// 양방향 유도를 정당화했는데, 그 컴포넌트는 UI 개편에서 **삭제됐다**(리포에 파일이 없고
/// 참조도 이 주석 한 줄뿐이다). 지금 프로덕션에서 그 두 축에 쓰는 자리는 아래
/// `applying(_:to:)` **하나뿐**이다(테스트 제외, `grep` 실측 2026-09-01).
///
/// 그래도 유도 방식은 유지한다. 이유가 "쓰기 주체가 둘" 에서 **"표현 불가 조합이 존재한다"**
/// 로 좁아졌을 뿐, 그 이유는 여전히 유효하다 — `criteria.types` 는 집합이라 사이드바 행
/// 하나로 못 그리는 값을 가질 수 있고(빈 집합/다중 선택), 저장한 선택은 그 순간 거짓말이 된다.
/// **아래 `selection(for:)` 의 `nil` 분기들을 "도달 불가라 죽은 코드" 로 읽지 마라** — 쓰기
/// 주체가 하나로 줄었어도 그 함수는 임의의 `ShellState` 를 받는 순수 함수이고, 테스트와
/// 미래의 두 번째 쓰기 주체가 그 분기의 소비자다.
///
/// ## 태그·나이 등급은 건드리지 않는다
///
