        ...
        if let d = quietAssetData(path, package: package),
           ...
           let p0 = (j["passes"] as? [Any])?.first as? [String: Any] {
            ...
            if let csv = p0["constantshadervalues"] as? [String: Any] {
                // **color 는 소문자가 정본 키다.** generic4.frag 의 `material:` 선언이 전부
                // 소문자이고, 코퍼스 실측으로 두 키를 다 가진 재질 289건(전건 generic4)이
                // 값이 다르며 대문자는 전부 `1 1 1`(기본값)이다 — 저작자가 칠한 값은 소문자에
                // 있다. 실증: 3470948192 의 'sd'(성단) 재질이 `Color:"1 1 1"` / `color:"0 0 0"`
                // 인데 WE 는 검게 그린다(반사만 보이는 블랙홀).
                //
                // ⚠️ **alpha 는 대문자 우선을 유지한다 — 뒤집으면 회귀한다(실측).** 둘을 같이
                // 뒤집었더니 3470948192 배경이 통째로 보라색이 됐다. 원인을 찾았다: 그 씬
                // `材质`(generic2) 재질이 `Alpha={user:"dopplereffect", value:0.02}` / `alpha=1`
                // 이다 — **대문자가 유저 프로퍼티 바인딩(0.02 반투명 베일)이고 소문자는 상수
                // 1(불투명)**이라, 소문자를 읽으면 베일이 불투명해져 배경을 덮는다.
                //
                // 그런데 코퍼스는 반대쪽이 더 많다: 두 키 값이 다른 19건 중 **소문자가 dict 인 것이
                // 7건, 대문자가 dict 인 것은 이 1건뿐**이고 나머지 11건은 둘 다 평문이다.
                // 즉 "대문자가 바인딩" 은 규칙이 아니라 예외다 — 근거 없이 일반화하지 마라.
                // 올바른 해법은 키 대소문자가 아니라 **바인딩(dict) 쪽을 우선하는 것**으로 보이지만
                // 그 규약을 WE 에서 확인하지 못했으므로 지금은 무회귀 쪽을 유지한다.
            ...
            if item.bb {
                ...
                for (meshIdx, mesh) in mr.meshes.enumerated() {
                    ...
                    if usedCustom, let custom = mesh.customShader {
                        ...
                        enc.setFragmentTexture(mesh.gradientTexture ?? mesh.texture, index: 2)
                        // M3/M①: PBR 노멀맵/마스크 텍스처 바인딩(mf_normal 전용 슬롯). 미보유 시 자기
                        // 알베도로 슬롯을 채우고(미바인딩 회피, gradientTex 선례와 동일), 소비 여부는 셰이더가
                        // normalParams.z(hasNormal)/y(hasMask) 플래그로만 게이트 — mask.r>0 값 기반 오폴백 제거.
