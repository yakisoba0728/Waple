...
public enum AudioSpectrum {
    ...
    /// 매핑의 불변식이 아니다 — 96 kHz(B=314)에서는 깨진다.
    ///
    /// **[정정 r4-14] 32 kHz 는 깨지지 않는다.** 종전 이 줄은 "32 kHz(B=940)나 96 kHz(B=314)
    /// 에서는 깨진다" 였는데, B=940 은 **상수 `topFrequency` 를 썼을 때의 값**이고 지금
    /// `binCount` 는 `engineTopFrequency(sampleRate:)` 를 쓴다(바로 위 `binCount` 주석이 그
    /// 폐기 사유를 정확히 적어 두었다 — 두 주석이 서로 모순돼 있었다). 실제로 재계산하면
    /// 32 kHz 에서 원본 N 이 1920 에 묶여 상한이 10650 Hz 로 내려가고 **B = 683** 이라
    /// `623...688` 안이다 = 29밴드 규약이 그대로 유지된다. 같은 이유로 44.1 kHz 이하는 전부
    /// B=683 이다(16 kHz·22.05 kHz 포함). 깨지는 것은 44.1 kHz **위**로 올라가 N 이 늘어나는
    /// 쪽뿐이다(96 kHz → 314).
