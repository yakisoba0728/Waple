...
public enum WapleProfiler {
    ...
    public private(set) static var phases: [String: Double] = [:]
    public private(set) static var counters: [String: Int64] = [:]
    ...
    public private(set) static var texRecs: [TexRec] = []

    /// makeLibrary 에 투입된 MSL 소스의 안정 해시(프로세스 간 비교 가능). 재컴파일 비율 산출용.
    public private(set) static var shaderHashes: [UInt64] = []

    /// 파티클 시뮬 스텝: (생존 파티클 수, 초). 정상상태 ms/프레임 + 피크 수 산출.
    public private(set) static var particleSteps: [(count: Int, seconds: Double)] = []

    /// 마운트 직후 MTLDevice.currentAllocatedSize(마운트가 enabled 시 기록).
    public static var deviceAllocatedBytes: Int = 0
