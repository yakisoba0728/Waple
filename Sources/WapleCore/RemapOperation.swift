...
/// 여기서는 **IEEE 정확 역수를 한 번 구해 곱한다**(`normalize` 의 `(raw − lo) * (1 / span)`) —
/// 헤드리스 결정성이 우선이고, 근사 테이블은 CPU 모델 의존이라 재현 자체가 옳지 않다.
///
/// r4-36 정정: 종전 이 줄은 "정확한 나눗셈을 쓴다" 고 적었지만 코드는 나눗셈이 아니라 역수
/// **곱**이다(두 텍스트의 출생 커밋이 `c2f4f3b0` 로 같다 — 작성 시점부터의 불일치). 역수 곱은
/// 나눗셈과 **비트동일이 아니다**: `1/span` 자체의 반올림이 한 번 더 들어가 무작위 표본의 약
/// 26%에서 1ulp 어긋난다. 결정성(같은 입력 → 같은 출력)은 둘 다 보장하므로 코드는 그대로 두고
/// 서술만 맞춘다 — 나눗셈으로 바꾸면 기존 골든이 1ulp 단위로 움직인다.
/// 폭이 1.0 인 기본 범위에서는 셋 다 정확히 1.0 이라 무회귀다.
