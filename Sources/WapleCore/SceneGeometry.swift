...
public enum SceneCameraMath {
    ...
    /// `delay > 3` 이면 음수가 되어 발산한다 — 실물 그대로다. 이 구간(≥3)에 닿는 자산은 없다.
    ///
    /// 도달(2026-09-01 재계수, `cameraparallaxdelay` 를 저작한 **씬 문서** 수 = 모집단):
    ///   · 동봉 `WEAssets` **168**개 — 전건 `0.1`.
    ///   · 설치본 `wallpaper_engine` **175**개 — `0.1` 174 · `1` 1(`projects/defaultprojects/ricepod`).
    /// r4-42 정정: 종전 표기 "`0.1` 176건 · `1` 1건 (동봉 168 + 설치본 175 씬 기준)" 은 파일 수와
    /// 값 도수를 뒤섞은 전치였다. 176 은 `grep -rl` 이 세는 **파일** 수인데(씬이 아닌
    /// `lib.sceneScript.d.ts` 1건이 섞여 있다) 그것을 `0.1` 의 값 도수 자리에 적었고, 175 는
    /// 설치본 씬 문서 수였다. 재현: 각 코퍼스의 씬 JSON 을 파스해 `general.cameraparallaxdelay`
    /// 보유 문서를 세고 값별로 분류한다(`{"value":…}` 바인딩은 언랩).
    ///
