...
- ~~**정적 개수와 CI 의 `Executed N tests` 는 지금까지 한 번도 어긋난 적이 없다.**~~
  ...
  **3,699** 가 나와 CI 실측 **3,708** 과 9 만큼 어긋난다. 즉 이 절이 기대는
  **"정적 개수로 하한 통과를 미리 계산한다"는 전제 자체가 그동안 깨져 있었다.** 최신 실측
  ...
  **여러 작업이 동시에 `Tests/**` 를 고치는 중이라면 하한에 여유가 0 이므로 푸시 직전에 다시 세라.**
  ...
  > **[2026-08-25 갱신] 일곱~열 번째 연속으로 어긋나지 않았다.** `7791a40`(둘 다 3,683) ·
  > `be3390a`(둘 다 **3,686**, run `32755977630`) · `f5d5d56e`(둘 다 **3,693**,
  > run `32776371263`) — 전부 debug·release 두 잡이 같은 수이고 정적 개수와도 일치.
  > 하한은 `3500` → `3686` → **`3693`** 으로 래칫했다.
  >
...
> **[2026-08-26] 정적 개수 레시피가 여러 벌이었고, 그중 둘이 9개를 조용히 흘리고 있었다.**
> 이 파일은 같은 수를 세는 "정본" 레시피를 **두 군데에 서로 다르게** 적어 두고 있었다 —
> 위 「빌드와 테스트」의 `'^\s*func test'`(앵커 있음)와 아래 [2026-08-20] 안의
> `'func test'`(앵커 없음)다. 같은 트리에서 전자는 **3,699**, 후자는 **3,708** 을 낸다.
>
> 차이 9는 전부 **`@MainActor func test…`** 형태의 선언이다 — 어트리뷰트가 `func` 앞에 붙어
> `^\s*func` 앵커에 걸리지 않았다. 실측 위치는 `SceneInteractionMediaE2ETests.swift` 7건
> (75·142·177·222·268·310·360) · `SceneSharedScriptTests.swift` 513 · `ParallaxControllerTests.swift` 129.
>
> **CI 하한 3708 쪽이 맞다**(`ci.yml` `Skip / execution census` — `Executed` 실측과 일치).
> 틀린 것은 문서 쪽이었고, 피해는 조용했다: 이 절이 내세운 **"정적 개수 = CI 실측"** 전제와
> 그에 기댄 **푸시 직전 하한 통과 사전 계산**이, 적힌 대로 따라 하면 항상 9 낮게 나와
> 여유 0 인 래칫에서 통과를 오판할 수 있는 상태였다. 위 "한 번도 어긋난 적이 없다" 문장은
> 그래서 취소선을 그었다 — 어긋난 적이 **있다**.
>
> 고친 것 셋. ① 정본 레시피를 어트리뷰트·수식어 허용 + 줄머리 앵커 유지로 바꿨다(실측 3,708 —
> 앵커를 유지해 주석 속 산문 언급은 계속 배제한다). ② 아래 [2026-08-20] 에 있던 경쟁 레시피는
> 걷어내고 정본 위치를 가리키게 했다 — **이 파일 안의 레시피는 이제 한 개뿐이다.**
> ③ `README.md` 의 `3,693` 을 **3,708** 로 맞췄다. `ci.yml` 은 손대지 않았다(맞는 값이다).
>
> **이 파일 밖에 있던 세 번째 판본도 같이 걷었다.** `'^\s*(public |private |internal )?func test'`
> 가 세 군데 더 있었다 — `.github/workflows/ci.yml` 404·469(둘 다 주석) ·
> `docs/dev/re-methodology.md` 380. 이 판본도 어트리뷰트를 앞에 두면 놓쳐 **3,699** 를 냈다
> (정본과 9 차이, 원인 동일). 게이트 값 자체는 3708 로 맞았으니 CI 동작은 정상이었고 틀린 것은
> 그 옆에서 세는 법을 가르치는 주석이었다. 셋 다 위 정본 한 줄로 교체했다 —
> `ci.yml` 의 게이트 식(`:513`)은 건드리지 않았다.
>
> **그래서 이제 리포 전체에 레시피는 한 판본뿐이다**(실측: 구 판본 grep 잔존 0건, 단 이 툼스톤이
> 인용하는 자리는 제외). 새로 적을 일이 생기면 베끼지 말고 위 정본 자리를 가리켜라 —
> 이 결함은 판본이 복제되면서 생겼다.
>
> 참고: 위 [2026-08-25] 이 "하한을 `3693` 으로 래칫했다" 고 적은 뒤 `ci.yml` 은 **3708** 로
> 한 번 더 올라갔다. 그 줄은 그 시점 스냅샷이고, 현재 하한은 늘 `ci.yml` 에서 읽어라.

> **[2026-08-27 래칫] 3723 → 3774 — 열두 번째 연속 일치.**
> run `33028115080` · 커밋 `e48bd3b`(브랜치 tip 자신, debug·release 두 잡 성공)에서
> `executed=3774 skipped=64` 이고 정적 개수도 정확히 3,774 다.
>
> 증분 51 은 전부 **재생정책 stage 2** 의 신설 오라클이고 삭제·통합은 0건이다:
> 전역면·병합·마스크 24(`PlaybackPolicyRuntimeTests`) · 관측자 6축 12(`PlaybackObserversTests`) ·
> 합류·엣지 추적 15(`PlaybackPolicyCompositionTests`). 배선과 심 타입 정정은 프로덕션 코드만
...
> `executed=3723 skipped=63` 이고 정적 개수도 정확히 3,723 이다.
>
> **단, 그 일치는 위 새 레시피로 셀 때만이다** — 종전 `'^\s*func test'` 는 같은 트리에서
> **3,714** 를 낸다(차이 9는 여전히 `@MainActor func test`). 이 라운드가 고친 것이 바로
> 그 사각지대이므로, 낡은 레시피로 세고 "9 모자란다" 고 놀라지 마라.
>
> 커밋별 증분은 `ci.yml` 의 하한 옆에 실측으로 적어 뒀다(추정 아님).
...
> 이 값은 **정적 개수**다(레시피 정본은 위 「빌드와 테스트」 절 한 곳뿐이다 — 여기 적혀 있던
> 별도 판본은 [2026-08-26] 에 걷어냈다). 마지막 CI 실측
...
> **[2026-08-27 → 정정 2026-09-01]** 종전 여기 붙어 있던 "지금 코드가 인용하는
> `analysis/decompiled/all/...` 은 리포에 없다" 는 정확하지 않았고, 그 다음에 적은
> "`Model3D.swift:110`·`ProjectJSONParser.swift:208` 이 `FUN_140261950` 을 가리킨다" 도
> **지금은 틀리다** — 그 두 자리는 2026-08-27 자로 참 VA(`FUN_140261880` ·
> `FUN_140046f20`)로 이미 교정됐고, 두 산출물 다 짝 저장소에 실재한다.
> `FUN_140261950` 이 `Sources/` 에 남은 것은 **2건**이고 둘 다 "종전엔 이 이름을 인용했다" 는
> **메타 설명문**이다(`Model3DFormat.swift` 의 `Model3DFormat` enum 주석, `Model3D.swift` 의
> `Model3D` 타입 주석) — 살아 있는 근거 인용이 아니다. `grep -rn 'FUN_140261950' Sources/`
> 로 확인해라. 현황은 [spec/README.md](spec/README.md) 의 같은 절이 정본이다 —
> **여기 두 번 적어서 또 썩었고, 이번이 두 번째다.** 이 절은 앞으로도 정본을 가리키기만 해라.
