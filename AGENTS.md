...
swift test                     # 2,293개(2026-08-19 CI 실측 — 코퍼스 유무와 무관)
swift run Waple                # 메뉴바 앱으로 실행
```

테스트 수 **2,293** 은 고정 기준값이다. 리팩토링으로 이 숫자가 변하면 무언가 잘못됐다.
...
`Skip / execution census` 가 실행 하한 2,293 과 스킵 상한 100 을 건다. 손으로 세어 적는
...
| 구성 | 실행 | 스킵 | 시간 | 출처 |
| --- | --- | --- | --- | --- |
| 코퍼스 있음(전량 460) | 2,293 | 2 | ~30분 | 실행수는 **추론**(정적 개수 = 축소 실측과 동일), 스킵 2 는 2026-08-01 실측 |
| 코퍼스 있음(축소 38, release) | 2,293 | 9 | **162초** | 시간·스킵은 2026-08-16 실측(`verify-plan-b12.sh` §5, `swift test -c release`, 순차). 실행수는 그 2,180 에 2026-08-17 신규 20 을 더한 **정적 추론** |
| 코퍼스 있음(축소 38, debug) | 2,293 | 9 | ~4.6분 | 시간·스킵은 2026-08-16 실측(`--parallel --num-workers 6`, 아래 레시피). 실행수는 위와 같은 **정적 추론** |
| 코퍼스 없음 | 2,293 | 40 | ~110초 | 2026-08-17 macOS 실측 (`WAPLE_REAL_PKGS=/nonexistent/path swift test`, 순차 — 번들별 25+1044+51+802+348) |
| CI (코퍼스 없음) | 2,293 | 46 | ~155초 | **전부 실측**(2026-08-19 CI run `32240720951`, macos-26). 종전 행은 실행수가 정적 추론이었고 "다음 CI 로 재확인할 것" 이라 적혀 있었다 — 그 재확인이 run `32222689131`(2,285/47)이었고, 이 행은 F530-sweep 이후 재측정이다. debug·release 두 레인이 실행·스킵·실패까지 동일했다 |

모든 구성 **실패 0**. `실행` 은 XCTest 의 `Executed N tests` 이고 **스킵을 포함한다** —
그래서 스킵이 40/46/9 로 갈려도 다섯 구성이 전부 똑같이 2,293 을 낸다. 위 `~110초`는 증분 빌드까지
...
(CI 는 번들을 `WaplePackageTests.xctest` 하나로 합쳐 2,293 한 줄로 낸다.)
