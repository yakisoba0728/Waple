...
swift test                     # 3,723개(2026-08-26 CI 실측 — 코퍼스 유무와 무관)
...
- 위 `3,038` 은 그 시점 스냅샷일 뿐 게이트가 아니다. 정확한 현재값은 직접 세라 —
  **정적 개수를 세는 정본 레시피는 아래 한 줄이다. 다른 데서 본 판본이 이와 다르면 그쪽이 틀린
  것이다**(2026-08-26 실측 **3,723** — CI `executed` 와 일치, 아래 [2026-08-26 래칫] 참조):
  ...
  `'func test'` 는 지금 트리에서는 같은 3,723 이지만 주석 한 줄에 흔들리고, 앵커만 건
