...
Engine 워크샵 프로젝트(`scene.pkg`, 동영상, 웹)를 데스크톱 배경으로 네이티브 재생합니다.
...
| `.webm` `.mkv` `.avi` 동영상 | 로컬 `ffmpeg`로 mp4 변환 후 네이티브 재생(WE 배경 패키지 내부 동영상 컨테이너 한정 — 원시 파일 드래그/드롭 가져오기는 `.mp4`/`.mov`/`.m4v` 만 지원) |
| `index.html` 웹 | 제한된 오프라인 WKWebView |

`.jpg`/`.png`/`.gif` 등 이미지는 최상위 배경 타입으로 지원하지 않습니다(타입 시스템·렌더러 팩토리에
image 케이스가 없음) — 씬(`scene.pkg`) *내부* 레이어에서 텍스처로 쓰이는 것과는 별개입니다.
...
전체 Wallpaper Engine 런타임 호환을 주장하지는 않습니다. 대부분의 미지원 씬 기능은 조용히 넘기고
로그를 남기지만, 일부 경로(예: 패키지에 바이트는 있으나 디코드에 실패한 레이어 텍스처, GPU 디바이스가
없는 환경의 셰이더 컴파일 검증)는 로그조차 없이 완전히 무음으로 스킵됩니다 — "로그를 남긴다"는
보장이 아니라 일반적인 설계 방향입니다.
...
swift test                 # 전체 테스트(1700+개) + 실물 씬 그라운드-트루스
...
- **라이브러리 메인창(SwiftUI, 항상 다크)** — 그리드 브라우저, 검색·필터(유형/태그/등급/즐겨찾기), 폴더 정리, 상세 패널(속성 편집·평점·제거), Now Playing 바, 디스플레이별 할당 화면.
- **검색·창작마당 탭** — Steam 워크샵 디스커버 레일 4종(인기 급상승·최신·구독순·평점순)과 검색·무한 스크롤 브라우저. steamcmd 다운로드 진행 UI, 타일 평점, API 키는 Keychain 보관.
- **설정 창 + 축소 트레이** — 화면 맞춤·가림 정지·재생목록·동영상 음량/배속·로그인 시작·정적 배경 동기화·화면보호기·기본 에셋을 설정 창 한 곳에 통합. 트레이 메뉴는 6항목(열기·최근 배경·일시정지·다음 배경·설정·종료).
...
Tests/           5개 타깃 · 1700+ 테스트(합성 단위 + 실물 코퍼스 그라운드-트루스)
