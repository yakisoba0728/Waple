...
| --- | --- | --- |
| GLSL → MSL transpiler | 🟡 | Source-to-source translation executed on the GPU; most effect shader variants in the measured local corpus compile. (**No percentage here on purpose.** A "~99.9%" figure stood in this cell from the README's first commit `76354e07` (2026-07-06) until 2026-09-01 with nothing behind it — no `spec/` entry, no `docs/re/` measurement and no `scripts/spec/measure_*.py` produces it; the only other mention in the tree is the audit note that flagged it. It is removed rather than replaced, because `docs/re/shader-translation.md` records that actual MSL compilation is unverified off-macOS (no Metal compiler on Linux) — a real rate has to be measured on a Mac before anyone quotes one.) Preprocessor (combos / `#include` / function-like macros), expression-level type inference (HLSL implicit vector truncation), GLSL structs, arrays, `inverse()`. Forward+ light *array* uniforms are registered but fed neutral scalars — no corpus scene enables those combos |
...
Tests/           7 targets (synthetic units + real-corpus ground truth). The test-count floor
                 lives in .github/workflows/ci.yml `Skip / execution census` — the one canon
