...
## 4-quater. 바이너리 단독 라운드 — 착지 8건 + 미착지 4건 (2026-08-20 오후)

워크샵 코퍼스가 없는 상태에서 **바이너리로 되는 것만** 골라 돌린 라운드다. Opus5 에이전트 8개를
병렬로 돌리고, **보고를 그대로 받아 적지 않고 결정적 주장은 전부 이 저장소에서 다시 떴다.**
아래 VA 인용은 전부 재확인분이다.
...
| 커밋 | 무엇이 거짓이었나 | 도달 |
|---|---|---|
| `418a343` | 호환성 스캐너가 렌더러와 **다른 방식으로 씬을 마운트**했다 | 동봉 씬 프로젝트 170/170 |
| `d09352f` | **난류가 위치가 아니라 속도에 누적된다** + 좌표·배수 사슬 전체 | 22파일 / 고유 18 |
| `7a56cc6` | `velocityrandom` 이 이미터 초기속도를 **덮었다**(실물은 더한다) | 3파일 / 고유 2 |
| `56b680c` | F628 테스트가 위치로 재고 있었다 (위 변경의 정확한 귀결) | — |
| `f512acb` | `inherit*valuefromevent` **섹션 오귀속** + 키 `value`→`input` | 2건 |
| `1fceb6a` | `maintaindistancebetweencontrolpoints` 미구현 | 실효 2건 |
| `61ad9bd` | `dtScaled` 가 어디에도 없었다 (7 오퍼레이터) | fps<40 에서만 발현 |
| `7dfb4c2` | **oscillate `frequency` 가 "수명당 횟수" 가 아니라 rad/s** (F832 반증) | 65 인스턴스(3종 합) |

특히 두 가지가 이 라운드의 성격을 보여준다.

**`418a343` — 양방향 거짓.** 스캐너의 계약은 "이슈 없음 = 렌더 가능" 인데, `.pkg` 가 없으면
그냥 빠져나가서 **언팩 씬을 한 건도 검사하지 않았다**. 그래서 (a) 문서가 깨진 언팩 씬이
무이슈로 통과했고(거짓 통과), (b) 이름이 관례가 아닌 패키지 씬은 렌더러가 정상으로 여는데
`.error` 로 단정했다(거짓 치명). 리눅스 드라이버 6케이스로 전/후를 대조해 둘 다 재현했다.

**`7dfb4c2` — 공식 문서보다 바이너리.** 종전 구현은 WE 디자이너 문서의 "oscillations per
particle lifetime" 을 근거로 삼고 있었다. 핸들러는 `age` 배열을 위상에 더한 뒤 freq 를 곱하고
(0x140240f75 → 0x140240f7a), 파스에는 2π 곱셈이 **없다**(0x1401cc655–0x1401cc7a0). 문서가
에디터 라벨을 설명할 뿐이다. 이 부류에서는 문서가 아니라 바이트가 정본이다.

### 미착지 — 조사는 끝났고 착지 조건만 남은 것

| 항목 | 확정된 내용 | 왜 아직 안 했나 |
|---|---|---|
| oscillate 공용 난수 | 파티클당 난수 1개(`[sys+0x338]`)를 freq·phase·scale 과 세 형제가 **전부 공유**, alpha/size 진폭은 그 난수로 스케일(`mulps xmm8, xmm9` @0x140240f67), Y축에만 `2π·r` 추가 위상 | 파티클당 드로가 최대 7→1 로 줄고 소비 위치도 앞당겨져 **난수열 전체가 이동** → Mac 골든 리베이스라인 필요 |
| `positionoffsetrandom` | 실물 키는 `directions`/`sign`/`scale`/`distance`/`timescale`/`octaves` 이고 **fBm 노이즈**다. `offsetmin`/`offsetmax` 는 `layerimage` **에미터**의 키를 오귀속한 것. RNG 드로 3 → **0** | 위와 같은 이유 + 실물 노이즈 커널(2D 심플렉스 추정) 미이식 |
| boids 내부 루프 | 안쪽 j 루프도 바깥과 같은 `K·4` 스트라이드로 서브샘플된다(`K = 최고수위/100 + 1`). 프레임당 쌍 수가 상수 1만 개로 묶인다 | 동봉 도달 0(전건 K=1)이지만 N>100 사용자 프로젝트에서 발현. 결과가 바뀌는 변경이라 화면 확인과 묶는 게 맞다 |
| CP `flags` bit2 | 자식 CP 를 **부모 파티클 시스템의 `parentcontrolpoint` 번째 CP** 에 부착(0x14022e66e → 0x14022e684). 자기참조 아님 | 도달 7(all)/3(non-preview). Waple 의 정적 CP 모델에 근사 이식은 가능 |

### 라운드 후반 추가 착지

| 커밋 | 무엇이 거짓이었나 | 도달 |
|---|---|---|
| `43b9cee` | boids **안쪽 j 루프가 전수 스캔**이었고 `K` 의 밑값이 생존 수였다(실물은 최고수위) | N>100 |
| `4695a6b` | CP `flags` bit2 미구현 — 자식 인력 중심이 **450 단위** 어긋나 있었다 | 실자산 1건 |
| `adf9674` | `positionoffsetrandom` 이 **`layerimage` 이미터의 키**를 읽고 있었다 | 5건 |

`43b9cee` 성능(리눅스 실측, 10프레임 평균): N=12000 에서 boids 비용이 프레임당 **6.09 ms →
노이즈 수준**(전 10.55−4.46 vs 후 4.05−4.72). 안쪽까지 서브샘플하면 프레임당 쌍 수가
`(N/K)² ≈ 10,000` 상수로 묶이기 때문이다 — `K = N/100 + 1` 의 `100` 이 그 예산이다.

`adf9674` 가 특히 나빴다. 읽던 키(`offsetmin`/`offsetmax`)가 동봉 5건 전건에 **없어서**
오프셋이 항상 0 이었고, 그러면서 파티클당 RNG 를 3번 낭비했다 — 원소가 시각적으로 통째로
죽은 채 난수열만 밀고 있었다. 실물은 fBm 노이즈 변위이고 RNG 드로가 **0** 이다.

### 남은 것 중 착지 조건이 분명한 하나 — oscillate 공용 난수

바이트로 확정됐다(전부 이 문서 기준으로 직접 재확인):
  · 스폰 VM 진입부가 난수를 **정확히 1회** 뽑아 `[sys+0x338][i]` 에 넣는다
    (`call 0x1401f87a0` @0x14023b372 → `movss [rax+r12*4], xmm0` @0x14023b381).
    이니셜라이저 디스패치보다 **먼저**다.
  · oscillate{position,alpha,size} 가 그 하나에서 freq·phase·scale 을 전부 파생하고,
    alpha/size 는 **진폭까지** 그 난수로 스케일한다(`mulps xmm8, xmm9` @0x140240f67 →
    꼬리 `addps xmm6, xmm12`(+1) → `mulps xmm6, xmm8` → `addps xmm6, [r14+0x50]`(smin)).
  · 세 형제가 같은 배열을 읽으므로 **서로 상관**된다(빠르게 흔들리는 입자가 빠르게 깜빡인다).

착지하지 않은 이유는 하나뿐이다 — **파티클당 드로가 최대 7 → 1 로 줄고 소비 위치도
이니셜라이저 앞으로 당겨지므로 난수열이 전역으로 이동한다.** `positionoffsetrandom` 처럼
해당 원소를 쓰는 시스템에 국한되는 게 아니라, 실물이 **분기 없이 항상** 뽑기 때문에
파티클을 쓰는 모든 씬의 골든이 흔들린다. 비용을 알고 결정할 일이라 남겨 둔다.

### 부수 산출물 — 리눅스 로컬 테스트 하네스

이 라운드에서 CI 가 잡아 준 실패가 세 번 있었고 전부 macOS 왕복 10분을 썼다. `simd` 와
`CFGetTypeID` 대역을 만들어 `WapleCoreTests` 를 리눅스에서 그대로 돌리게 했다 —
`scripts/dev/linux-core-tests.sh`, 실측 **946개 / 약 9초 / 실패 0**(macOS CI 결과와 일치).
렌더 계층·골든은 여전히 범위 밖이지만, 이 브랜치에서 CI 가 잡은 실패는 **전부 이 범위 안**이었다.
