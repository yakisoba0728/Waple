...
HEAD 회귀(구 베이스라인 대비)는 3걸뿐 — 나머지는 전부 기존 결함(§7).
...
- ~~3D 씬 텍스트 미배선~~ **해소(W-①, `08058c9`)**: build3D 가 doc.texts → TextRasterizer(2D buildTexts 와
  ...
  전 씬 결정적self-check 통과) — 3662790108·3477054430·3737268876 은 byte-identical(frac=0.0000, 새
...
| 텍스처 오바인드 | 3492627662, 3413921910(grasses) | 파트/노이즈 밴드 |
| foliagesway 3연 체인 | 3544823846 | 레이어 통째 드롭 |
| 스프라이트시트 NPOT .tex | 3407317466 | 디코드 실패(RGB 스트라이프) — **정정(2026-07-27, B3-3d-video "하늘 회색
  블롭" 조사)**: 같은 씬 3407317466 의 "하늘 회색 블롭/글리치" 증상은 main-6526db1·본 라운드 재캡처
  모두에서 재현 실패(하늘이 정상 그라디언트로 렌더, 다른 웨이브에서 이미 해소된 것으로 추정) — **단
  이 행이 서술하는 NPOT 스프라이트시트 디코드 실패 자체는 재검증하지 않았다**(별도 증상일 가능성 있음,
  기각 아님). 머지 시 B4 갱신 문서와 대조해 정리 필요. |
| composelayer 오프스크린 | 3521337568, 3565190341 | 불투명 합성/검은 타원 |
...
| 씬 | 내용 |
|---|---|
| 3489263099 | Rain 파티클 정상→블롭(regressed+garbled, high) |
| 3563096027 | tint R 채널 누수: (74,21,74) vs 베이스라인 (20,19,74) — BLENDMODE 12 용의 |
| 3465215190 | 빗줄기 스프라이트 굵고 밝게(medium confidence) |
