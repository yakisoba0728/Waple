...
- **[2026-08-26 갱신] 아래가 말하는 `SceneRenderer.capturePointerUV` 정적은 지금 없다** —
  `ae7fa12` 에서 인스턴스 프로퍼티로 옮겼다(전역이라 데이터레이스·라이브 오염·비재진입 복원이
  동시에 성립했다). 아래 서술은 2026-08-02 시점 기록으로 남긴다.
...
(a) 가 이 항목의 선행이었는데 그쪽이 **원인 수정까지 끝나서**(포인터 핀) 새 기준선은
세션 간 잡음 없이 떴다 — 설치 게이트가 그것을 실측으로 확인한다(커서를 옮긴 뒤 재캡처 0종).

### (c) HDR 블룸 필터 교체 — **끝났다(2026-08-02)**

WE 평문 구조(4탭 ±0.5 소스 텍셀 · 가우시안 패스 없음 · 4탭 additive 업샘플 · 4탭 합성)로
...
- **함정 하나**: 셰이더 문면대로 업샘플 가중을 `평균 × scatter` 로 두면 레벨마다 1.619배가
  ...
  셰이더 `g_BloomScatter` 로 그대로 가는지 미확인이라 **가중은 종전 캘리브(0.25 × scatter) 유지**,
  탭 모양만 WE 로 맞췄다. 미해결 항목으로 `engine.bloom.hdr.upsampleWeightUnknown`(추정) 기록.
- LDR 블룸은 여전히 3패스(추출→블러→합성)다. WE 평문에 `blur_h_bloom`·`downsample_quarter_bloom`
  이 있으므로 같은 방식으로 대조 가능하지만, 어느 셰이더 조합이 LDR 경로인지 정본이 없다.
...
7. **`f3a17da` 이전에 뜬 캡처는 세션 간 대조에 29종의 잡음이 섞여 있다**
   (`spec/golden/nondeterminism.json` — 캡처가 마우스 커서 위치를 픽셀에 구웠다).
   원인은 수정됐지만 **옛 산출물은 그대로 오염돼 있다** — 그 이전 캡처를 지금 것과 비교하지 마라.
   새로 뜬 것끼리는 세션이 갈려도 안전하다(재베이스라인 게이트가 실측으로 확인한다).
