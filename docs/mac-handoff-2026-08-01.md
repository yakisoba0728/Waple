...
- 원인은 아직이다(`oracle.nondet.cause`, 상태 `추정`). 배제된 것: 바이너리 동일성·부하 개입·
  단건 대 순차·TZ·CWD·디스크 입력·절전/재부팅. 다음은 게이트 매트릭스 —
  `WAPLE_BC_NATIVE=0` · `WAPLE_DISABLE_TRANSLATED=1` · `WAPLE_LAYER_TRUNC=n` · `WAPLE_NO_BLOOM`
  을 상태마다 떠서 **상태 간에도 동일해지는 설정**을 찾는다(그 설정이 끄는 경로가 원인).
  상태는 30분~1시간 단위로 저절로 바뀐다 — 플립을 기다렸다 같은 매트릭스를 다시 뜨면 된다.
