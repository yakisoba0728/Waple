...
| `baseline-f3a17da` (2026-08-02, 현행) | **release** | **170 캡처 + 0 empty** | **166s (2.8분)** | 1.0s |

debug 오버헤드가 씬당 2.4배다. `empties` 가 24 → 0 으로 바뀐 것은 H1 수정으로
비디오-백드 씬이 `entries` 에 들어왔기 때문이지 성능 변화가 아니다.

`baseline-f3a17da` 의 1.0s/씬은 **mp4 캐시가 더워진 상태**의 값이다(같은 세션에서 이미 여러 번
...
대응은 lax 강등)와 **비결정 씬 1종**(`3363252053`).
