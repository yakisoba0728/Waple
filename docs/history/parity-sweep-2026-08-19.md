...
| G-A5-05 / G-B2-03 `[짝]` **✅ `f1ba768`+`77ec33a`** | `unique`/`clear` 미모델링 + `swap` 이 프레임-로컬 | flags bit0=unique/bit1=clear(`0x1401e7862`); `clear` 보유 6 fbo(번들 **최상위** 기준 — preview 사본까지 세면 12)가 **전건** unique | `unique` 코퍼스 81 |
| G-A5-07 / G-B2-05 **✅ `4785c0d`** | `conditions`(fbo/bind/pass) 미평가 | 평가기 전문 확정: op 는 `ge/gt/le/lt` **4종뿐**, 누산 전부 AND, 미정의 콤보 = 0 | fluidsimulation |
| G-B2-08 / G-B4-01 **✅ `e6cfbfb`** | 이펙트-로컬 자산 루트 부재 | 스톡 46종이 `effects/<name>/` 아래 자기 루트를 갖는데 Waple 은 팩 루트만 본다 → **베이스팩 폴백으로 46/46 해석 불가** | 잠재 + 번들 사장 |

이 넷을 고치면 `fluidsimulation`(20패스·9 FBO) 하나가 통째로 살아나고, `motionblur` 누적이
우연이 아니라 계약이 된다. **G-B2-02 없이 G-B2-03 만 고치면 유체는 여전히 안 돈다.**

### 2.2 오디오 — 한 커밋으로 묶어야 하는 4건 `[짝]` **✅ `a84cc9f`+`46db843`**
...
| ID | 갭 | 도달 |
| --- | --- | --- |
| G-C3-02 `[짝 G-E3-04]` **✅ `3a369eb`** | `.mdl` **MDLV0004/0014 전면 거부** | WE 설치본 `.mdl` **28개 중 23개(82%)**. 기본 3D 배경 9개 중 **8개가 모델 0개**(techno 만 4개 중 3개가 v0023 이라 부분 생존) |
| G-C3-05 | `skinCount > 1` **선택** 미지원 | audiophile 바닥 그리드 1건. `3a369eb` 이후 파서는 `Mesh.materials` 로 전 스킨을 **보존**한다 — 남은 건 씬의 `"skin": N` 선택뿐이라 렌더 측 변경이다 |
...
| `46db843` | 오디오 **소비단** 스테이지(그룹 정규화 · 1-pole · 슬루 제한 · 64→32→16 MAX 축약) — 레벨이 3.5~5배 모자랐던 진짜 이유 |
...
| `4785c0d` | `conditions` 평가기 — fbo·pass·bind 를 이펙트 인스턴스 `combos` 로 켜고 끈다 |
...
### 6.5 `.mdl` v0004/v0014 — 끝까지 확인한 결과

파서만 고치고 "됐다" 고 하면 안 되는 부류라, 기본 3D 프로젝트 전체를 씬 파일부터 따라가
실제로 지오메트리가 나오는지 A/B 했다(리눅스에서 `simd` 스텁을 만들어 실제 컴파일·실행;
`project.json` 의 `file` 로 씬 이름을 찾고 — 넷은 `scene.json` 이 아니다 — 모델 json
간접참조까지 추종).
...
패치 전의 3건은 전부 techno 의 v0023 이다 — 원장이 적은 "9개가 통째로 안 그려짐" 은
프로젝트 단위로는 8개가 맞고 techno 는 4개 중 1개(v0014 sphere)만 잃고 있었다.

짝 항목 G-E3-04(`general.fov` 부재 → 3D 카메라 소실)는 `7c8d1f1` 에서 이미 닫혔으므로,
이 둘이 함께 서야 한다는 조건도 충족된다 — 파스된 메시가 실제로 화면 좌표를 얻는다.

### 6.4 남은 것

§2.1 의 마지막 항목 `conditions`(G-A5-07/G-B2-05) 도 `4785c0d` 로 착지했다 — **§2.1 은 네 건이 전부 닫혔다.**
좌변 출처가 **이펙트 인스턴스 레벨 `combos`** 인데 동봉 씬 57개에서 그 키는 **0건**이고
(전부 패스 레벨 `combos` 60건), 그래서 원본에서도 fluidsimulation 의 `LIGHTING==1` ·
`RENDERING==3` 은 전부 false 다. 즉 충실한 구현은 조명/노멀 패스를 **끄는** 방향이고,
구현도 그렇게 떨어진다.

이 라운드가 손대지 않은 것은 §2.3~2.7 이다 — 파티클 오퍼레이터 12종 · `baseclasses.js` ·
`.mdl` MDLV0004/0014 · 라이팅/블렌딩 · 미디어. 거기가 다음 라운드의 입구다.
