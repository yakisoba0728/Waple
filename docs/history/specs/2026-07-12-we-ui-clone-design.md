...
- 셸(스크린샷 실측 확정, 2.8.42 한국어판):
  - 타이틀바: 좌측 상태 텍스트 영역(신호등과 공존), 중앙 타이틀, 우측 파란 `»` 우측패널 토글.
  - 탭 3개: **설치됨▼ · 검색 · 창작마당** + 우측 상단 버튼 3개: **모바일 · 디스플레이 · 설정**(탭 아님).
  - 검색줄: 검색창 · "필터 적용 결과" 파란 토글 · 정렬방향 ▲ 토글 + 정렬 콤보.
  - 하단 바: "재생목록" 라벨 + 불러오기/저장/구성/배경화면 추가 버튼 줄, 그 아래 **배경화면 편집기(비활성+툴팁) · 배경화면 열기(=임포트)** 대형 버튼 2개.
  - `WEStatusBanner`.
- 창: `fullSizeContentView`/투명 타이틀바/신호등 유지, 기본·최소 크기는 WE 실측.
- 미커밋 `WAPLE_SMOKE` 훅(main.swift·AppDelegate) 정식 커밋 — 이후 전 SP의 검증 도구.

### SP2 — 설치됨 탭
- 좌: 접이식 필터 사이드바(스크린샷 확정): "필터 초기화" 버튼 / "다음만 표시"(승인됨·내 즐겨찾기·모바일 호환·
  오디오 응답형·커스텀 가능) / 접이식 섹션(유형·나이 등급·해상도·출처·태그 — 태그에 "전체/없음" 일괄 링크).
- 중: 그리드 + 하단 중앙 페이지네이션(« 1 2 3 4 5 », WE 실측 페이지당 개수). 타일: 호버 gif·하단 제목 스트립·
  승인됨 트로피 배지·**적용 중 타일 파란 오버레이+X(중지)**·**폴더 타일**(라이브러리 폴더).
- 우: 선택 패널(스크린샷 확정 순서) — 프리뷰 → 제목 → ?·연령판정 → ★별점+♥즐겨찾기 → 타입·크기 →
  칩(태그·해상도·등급·커스텀) → 구독 취소(빨강 = 라이브러리 제거 매핑) → 코멘트(비활성)·복사·신고(비활성) →
  속성 편집(기존 `PropertyControl.kind` 매핑 재사용, 겉만 WE 컨트롤) → 하단 확인/취소.
- **신규: 라이브러리 제거**(`LibraryStore.remove(id:)`) — playlist/monitor orphan id 정리 동반
  (BACKLOG "LibraryStore.remove 부재" 항목 해소).
- **신규: 라이브러리 폴더**(`FolderStore` — 로컬 폴더에 엔트리 배정, 그리드에 폴더 타일·진입/탈출).
...
### SP4 — 검색(디스커버) + 창작마당 탭
- 검색 탭: 기존 `WorkshopClient` 정렬 쿼리(트렌드 3·최신 1·구독순 9·투표 0)를 행별로 조합해 WE 검색 탭 레이아웃 재현.
- 창작마당 탭: 검색+정렬+페이지네이션(`page` 파라미터 기존재)·다운로드 UI(기존 `DownloadUIState` 재사용).
- 모바일 버튼(우측 상단): 표시하되 비활성 + 툴팁(페어링 프로토콜 재현 불가).
...
| 라이브러리 폴더 | `FolderStore`(folders.json) — 폴더명→엔트리 id 목록, 동일 패턴 |
...
| 즐겨찾기·별점 표시·태그/등급 필터·페이지네이션·항목 제거·라이브러리 폴더 | 신규 구현(위 표) |
| 구독/구독취소 | steamcmd 다운로드 / 라이브러리 제거로 매핑 |
| 모바일 버튼 | 비활성 + 툴팁 (페어링 프로토콜 불가) |
| 코멘트·신고 버튼 | 비활성 + 툴팁 (Steam 세션 불가) |
| 타이틀바 "Steam을 사용할 수 없습니다" 상태 | steamcmd 세션 상태 표시로 매핑 |
...
| 시점 | 필요한 스크린샷 |
| --- | --- |
| SP1 (착수 전) | ✅ 수령(2026-07-12): 메인창 전체 — 필터 닫힘/열림 2장(타이틀바·탭바·검색줄·하단 바·우측 패널·페이지네이션 포함) |
| SP2 | ✅ 대부분 수령(위 2장). 추가 희망: 타일 호버 상태, 정렬 드롭다운 열림, 설치됨▼ 드롭다운 열림, 폴더 진입 화면 |
