...
### Task 8: 의도적 이탈 3건 — 결정하고 명시한다

**이건 사람의 결정이 필요하다.** 세 건 모두 `[0,1]` 입력에서는 등가고
**HDR >1 / 음수 입력에서만** 갈린다.

| # | 위치 | Waple | WE |
| --- | --- | --- | --- |
| 1 | `ScenePBRLighting.swift:7,15` · `Mesh3DShaders.swift:171` | GGX 분모 `max(raw, 1e-4)` | 바닥값 없음 |
| 2 | `ScenePBRMath.geometry` | `nv`/`nl` 을 `max(_, 0.001)` | (분해 결과 확인 필요) |
| 3 | 블렌드 엡실론 가드 | `step/select + 1e-5` | 정확 비교 `blend==0.0`/`==1.0` |

- [ ] **Step 1: 분해 결과에서 WE 실제 동작을 확인**

`spec/engine/shaders.json` 의 PBR·블렌드 항목을 읽는다. 세 건 각각에 대해
WE 원문이 무엇인지 확정한다.

- [ ] **Step 2: 사람에게 결정을 받는다**

선택지는 둘이다:
- **정본 일치로 제거** — HDR 씬에서 WE 와 같아진다. NaN/Inf 위험을 감수한다.
- **안전장치로 유지** — 지금 동작을 지킨다. HDR 씬에서 WE 와 갈리는 것을 감수한다.

**셋을 묶어 한 번에 정한다.** 따로 정하면 규약이 쪼개져 나중에 재발굴된다.

- [ ] **Step 3: 결정을 `spec/` 에 명시**

`spec/engine/deviations.json` 을 만들어 세 건 각각에 결정과 근거를 기록한다.
`status` 는 결정이 근거를 갖는지에 따라 `확정`/`추정`.

- [ ] **Step 4: 결정대로 코드 반영 + 테스트**

제거를 택했다면 HDR 씬(코퍼스 `hdr=true` 16종)에 대해 `--compare` 로 영향을 본다.
유지를 택했다면 코드는 그대로 두되 **주석에 "의도적 이탈, spec/engine/deviations.json 참조"** 를 남긴다.

- [ ] **Step 5: 커밋**

```bash
git add spec/engine/deviations.json Sources/ Tests/
git commit -m "의도적 이탈 3건 결정 반영(GGX 바닥값·nl 하한·블렌드 엡실론 — spec 에 근거 명시)"
```
