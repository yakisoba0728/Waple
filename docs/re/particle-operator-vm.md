...
> **[2026-08-28]** `+0x10`(`fadeouttime`)은 **페이드아웃 시작 시각**이지 지속시간이 아니다.
> 같은 슬롯이 `0x1402402f9 cmpltps xmm1, [r14+0x10]` 에서 **비교 피연산자로도** 쓰인다
> (`fout < t`). 식 전문과 3분기 구조는 §7 표 아래 정정 블록 참조.
...
| 3 | `t = age/lifetime` 으로 alpha(`+0x310`)에 **3분기 select** `f = (t < fin) ? t·rcp(fin) : (fout < t) ? (1−t)·rcp(1−fout) : 1.0` 를 곱한다. **`fadeouttime` 은 지속시간이 아니라 페이드아웃 시작 시각이다.** 역수는 `rcpps`(`0x1402402b4`,`0x1402402c7`)로 런타임 계산. **[2026-08-28 정정 — 아래 블록]** |
| 4 | `u = clamp01((t − starttime)·rcpDur)` 로 size(`+0x270`)에 `start + delta·u` 를 적용한다. 프롤로그가 매 프레임 `+0x278`(기준 size)에서 되돌린 뒤라 누적되지 않는다. |
| 5 | 같은 `u` 로 `start + delta·u` 를 내고, 그것을 color rgb(`+0x2f8/+0x300/+0x308`)에 **곱해 되쓴다**(대입이 아니다). **[2026-08-28 정정]** |
| 6 | 같은 `u` 로 `start + delta·u` 를 내고, 그것을 alpha(`+0x310`)에 **곱해 되쓴다**(대입이 아니다). **[2026-08-28 정정]** |
...
> **[2026-08-28 정정 · opid 3] `alphafade` 는 두 `min` 의 곱이 아니라 3분기 select 다.**
>
> 종전 기재: `min(1, t/fadein) · min(1, (1−t)/fadeout)`.
> 실물:
>
> ```
> f = (t < fin)  ? t · rcp(fin)
>   : (fout < t) ? (1 − t) · rcp(1 − fout)
>   :              1.0
> ```
>
> **가장 중요한 차이: `fadeouttime` 은 페이드아웃 *지속시간* 이 아니라 페이드아웃이
> *시작되는 시각* 이다.** 그래서 감쇠 기울기의 분모가 `fadeout` 이 아니라 **`1 − fout`** 이다.
>
> 바이트:
>
> | VA | 명령 | 역할 |
> | --- | --- | --- |
> | `0x1402402af` | `subps` | `1 − t` (그리고 `1 − fout`) |
> | `0x1402402b4` | `rcpps [r14+0x10]` | `rcp(1 − fout)` — `+0x10` = `fadeouttime` 레코드 슬롯 |
> | `0x1402402c7` | `rcpps` | `rcp(fin)` |
> | `0x1402402ef` | `cmpltps` | `t < fin` |
> | `0x1402402f9` | `cmpltps xmm1, [r14+0x10]` | **`fout < t`** — 상수와 비교하는 게 아니라 레코드의 `fadeouttime` 과 직접 비교한다. 이것이 "시작 시각" 의 직접 증거다 |
> | `0x140240308` | `andnps` | 어느 분기도 아니면 `1.0`(`xmm12` ← `0x140483640` = `(1,1,1,1)`) |
> | `0x140240322` | `mulps` | 기존 alpha 에 `f` 를 곱해 되쓴다 |
>
> **왜 이제껏 안 드러났나:** 두 식은 **`fout == 0.5`(기본값)에서만 항등**이다.
> `fout = 0.5` 면 `1 − fout = 0.5 = fout` 이라 분모가 우연히 같아진다. 출하 자산 대부분이
> 기본값을 쓰므로 종전 식으로도 결과가 맞아떨어졌다. `fadeouttime` 을 만진 벽지에서만 갈린다.
>
> **자기확증:** 같은 문서 §"opid 3 `alphafade`" 가 `rcpps [r14+0x10]` @`0x1402402b4` 를
> 이미 싣고 있고 거기서 `+0x10` = `fadeouttime` 이라 못박았다. `0x1402402f9` 의
> `cmpltps xmm1, [r14+0x10]` 은 **같은 슬롯을 비교 피연산자로** 쓴다 — 레코드 매핑이
> 두 방향에서 맞물린다.

> **[2026-08-28 정정 · opid 5·6] "채운다" 가 아니라 "곱한다".**
>
> 종전 기재는 `start + delta·u` 를 채널에 **대입**하는 것처럼 읽혔다. 실물은 그 값을
> **기존 버퍼 값에 곱해 되쓴다**:
>
> | VA | 명령 | 채널 |
> | --- | --- | --- |
> | `0x14024042f` | `mulps xmm2, [rdx+rcx*4]` | color — 기존 값을 읽어 곱한다 |
> | `0x140240433` · `0x140240436` | 〃(연속) | color 나머지 2채널 |
> | `0x1402404a8` | `mulps` | alpha |
>
> 실질 차이: 같은 파티클에 `colorchange`/`alphachange` 와 다른 색 오퍼레이터(또는 이니셜라이저
> 색)가 함께 걸리면 **결과가 곱으로 합성**된다. 대입이라면 뒤엣것이 앞엣것을 덮어썼을 것이다.
