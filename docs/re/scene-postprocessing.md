...
>
> **[2026-08-21 갱신 — W-17 단계 1 해소]** `539d77f` 가 볼류메트릭 **씬 뎁스 클립**을 넣었다.
> 조사해 보니 `.dontCare` 를 읽는 조용한 파손은 **없었다** — 갓레이가 씬 뎁스를 **아예 안 읽고**
> 있었고 샤프트가 지오메트리를 통과했다. 처치는 pooled depth 에 `.shaderRead` 를 주고
> (캐시 키가 크기뿐이라 **무조건** 준다 — 조건부는 "먼저 만든 쪽이 이기는" 버그다),
> `needsDepthStore` 에 볼류메트릭을 넣고, MSL 이 `tExit = min(tExit, sceneLimit)` 로 자르게 한 것이다.
> 클리어 뎁스 1.0 이 `farZ` 이상으로 풀려 `min` 이 무연산이므로 **지오메트리 없는 기존 픽스처는
> 비트 동일**이다. 나머지 단계(라이트버퍼 RT 세 장 + blur3 h/v)는 여전히 미구현이다.
> 같은 커밋이 **스팟 콘 변환기가 두 벌이던 것**도 한 벌로 접었다(`docs/re/scene-lighting.md` §10) —
> 저작 20°/30° 라이트가 갓레이에서는 10°/15° 로 그려지고 있었다.
>
