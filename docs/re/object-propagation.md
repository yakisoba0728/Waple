...
| # | 주장 | 판정 | 확신 |
|---|---|---|---|
| 1 | `disablepropagation` 문자열은 **`wallpaper64.exe`(64비트 엔진) · `wallpaper32.exe`(32비트 엔진) · `wallpaperui.exe`(에디터)** 세 호스트에 있다. `webwallpaper64.exe` · `scenescript64.dll` 은 0건 | — | **확정 → 2026-08-28 정정** — 아래 호스트별 실측표. 종전 이 행은 `wallpaper32.exe` 를 빠뜨리고 "다른 바이너리 0건" 이라 적었다(§1.1 표는 처음부터 맞았다 — §0 요약만 틀렸다) |
...
> **[2026-08-28 · 호스트별 VA 확정]** 위 표는 파일 단위라 VA 가 없었다. 주소까지 박아 둔다 —
> §0 결론 1행이 `wallpaper32.exe` 를 빠뜨린 원인이 "요약할 때 32비트 엔진을 64비트와 같은
> 칸에 묶어 놓고 잊은 것" 이기 때문이다.
>
> | 호스트 | `disablepropagation` | VA |
> | --- | ---: | --- |
> | `wallpaper64.exe` | 1 | `0x140490338` |
> | `wallpaperui.exe` | 1 | `0x140ad8540` |
> | **`wallpaper32.exe`** | 1 | **`0x7de948`** (32비트 이미지라 VA 폭이 다르다) |
> | `webwallpaper64.exe` | **0** | — |
> | `scenescript64.dll` | **0** | — |
>
> **§2 이하의 분석은 전부 `wallpaper64.exe` 기준**이고 그대로 유효하다 — `wallpaper32.exe` 는
> 같은 소스의 32비트 빌드라 새 사실을 주지 않는다. 고친 것은 부재 주장의 **모집단 라벨**뿐이다.
