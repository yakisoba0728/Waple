...
| **G11** | **`keepaspect` 소비처가 없다** | `SceneDocument.materialUserTextureKeepAspect` 선언부("파스·보존 전용") | §2.1 — `videoplayer` 1건 | **하** | 유저 텍스처 슬롯 바인딩에서 UV 스케일 한 번. 도달 1건 |
...
| 난이도 | 항목 | 공통 사유 |
| --- | --- | --- |
| **하** (7) | G3 G4 G5 G6 G9 G11 G12 G15 | 상수·플래그·한 줄 배선. AVFoundation 에 이미 대응물이 있다 |
