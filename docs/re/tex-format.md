...
`w·h·depth·4 ≤ 0xffffffff` 를 어기면 ~~에러 경로~~ **프로세스 즉사**(`0x1400986c0`)다.

> **[2026-08-28 정정] `0x1400986c0` 은 "에러 경로" 가 아니라 `FatalAppExitA` 다 — 프로세스가
> 그 자리에서 죽는다.**
>
> `FUN_1400986c0` 의 본문은 `vsnprintf` 로 메시지를 만든 뒤 **`FatalAppExitA`** 를 부르는 것이
> 전부다. 반환하지 않는다. 예외도, 폴백 텍스처도, 로그 후 계속도 아니다 — **`.tex` 파일 하나가
> 벽지 프로세스 전체를 내린다.**
>
> **어디가 즉사 지점인가 — 전수 실측.** `.text` 전 영역의 `E8` 상대호출을 전부 디코드해
> `0x1400986c0` 을 목적지로 갖는 자리를 셌다: **호출부 정확히 9개 / 함수 6개**.
> 그중 **3개가 TEXB(이미지 블록) 리더 범위** 안이다:
>
> | 호출부 VA | 맥락 |
> | --- | --- |
> | `0x14015d3f8` | 위 경계 검사 실패(`w`/`h`/`depth`/곱 오버플로) |
> | `0x14015df56` | TEXB 블록 |
> | `0x14015dfbf` | TEXB 블록 |
>
> **나머지 `.tex`/`.pkg` 리더 범위에는 0개다:**
>
> | 범위 | 즉사 호출부 |
> | --- | ---: |
> | `TEXI` 헤더 리더 `0x14015c760` | **0** |
> | `TEXS` 스프라이트시트 리더 `0x14015e1d0` | **0** |
> | 섹션 루프 `0x14015e580` | **0** |
> | `.pkg` 리더 범위 | **0** |
>
> ⇒ **TEXB 가 `.tex` 파싱에서 유일한 즉사 지점이다.** 나머지 섹션은 손상돼도 프로세스를
> 안 내린다. 이것은 적대적 입력 서술에서 반드시 구분해야 하는 차이다 — "파싱 실패" 의
> 결과가 섹션마다 다르다.
>
> Waple 대조: Waple 의 `.tex` 리더는 어느 경로에서도 프로세스를 내리지 않는다(던지거나 `nil`).
> **이 차이는 이탈이 아니라 개선**이고 되돌릴 이유가 없다 — 다만 "WE 와 같은 입력에 같은
> 결과" 를 재는 테스트에서는 WE 쪽 기대값이 **크래시**임을 알고 있어야 한다.
