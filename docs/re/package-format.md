...
| 최대 경로 깊이 | **슬래시 6개 = 7 성분** | 8종 (아래) |
| 최대 이름 길이 | 266 B | — |

최상위 디렉터리: `materials` 7,137 · `models` 2,320 · `shaders` 664 · `sounds` 455 · `effects` 281 ·
`particles` 278 · `fonts` 172 · `scripts` 30 · 루트 직속 1.

> **[정정 2026-09-01] "6 성분" 은 슬래시 개수를 성분 개수로 적은 것이었다.** 실측하면 최대
> 슬래시가 6개이고 성분은 그보다 하나 많은 **7**이다. 깊이 7 인 경로는 8종이고 전부
> 워크샵 배경이 **다른 워크샵 배경을 통째로 품은** 중첩 구조다:
>
> ```
> materials/workshop/2779107487/workshop/2084198056/effects/Simple_Audio_Bars.json
> └1───────┘└2─────┘└3────────┘└4─────┘└5────────┘└6─────┘└7──────────────────┘
> ```
>
> 성분 분포(전 11,338 경로): 2성분 5,730 · 3성분 2,182 · 4성분 2,087 · 5성분 1,330 ·
> **7성분 8** · 1성분 1 — **6성분은 0건**이다. 즉 옛 표기는 실재하지도 않는 깊이를 적고 있었다.
> 경로 깊이로 버퍼·재귀 한계를 잡는 코드가 있다면 6이 아니라 7로 잡아야 한다.
>
> ```bash
> # 짝 저장소에서 재현
> python3 -c "
> ps=[l.rstrip(chr(10)).split(chr(9),1)[1] for l in open('corpus_scan/entry-name-frequency.tsv',encoding='utf-8').readlines()[1:] if l.strip()]
> print('max slashes', max(p.count('/') for p in ps), 'max components', max(len(p.split('/')) for p in ps))"
> ```
