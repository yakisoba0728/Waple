...
그리고 파스는 `asUInt`(`0x140085f70` @`0x1401ce831`) 직독이라 **`isNumeric` 게이트가 없다**
...
| 사실 | 확인 방법 | 결과 |
| --- | --- | --- |
| `flags` 는 `[op+0x1c]` ← `asInt` 직독 | 파서 `0x1401c5490`–`0x1401d152c` 선형 | `operator[]` `0x1401ce829` → `asUInt`(`0x140085f70`) `0x1401ce831` → `mov [rsi+0x1c], eax` `0x1401ce83d`. **`isNumeric` 게이트 없음** |
