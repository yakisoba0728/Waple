...
2. **머티리얼 패스의 `combos` 키는 엔진이 무조건 대문자로 바꾼다**(`0x140154598` 의
...
### 2.2 키는 **무조건 대문자로 바뀐다** (`0x140154598`)
...
콤보 경로는 `0x140154598` 단 하나다.

이펙트 **인스턴스** 레벨 `combos`(`objects[].effects[].combos`, `conditions` 의 좌변)는
`0x1401e7319` 에서 따로 읽히고 **대문자화하지 않는다**. 즉 대문자화는 "셰이더로 주입되는
패스 콤보"에만 걸린다.
...
**[미해소 2026-08-21 — 좁아진 것]** 병합 자리는 여전히 못 짚었다. 다만 두 가지가 정해졌다:
(a) 리스트는 **정렬되지 않는다**(§3.2) — 즉 "합치는 순서" 가 곧 "방출 순서" 이고, 텍스트
전처리라 **뒤가 이긴다**. (b) 그래서 "저작값이 기본값을 이긴다" 는 계약은 *덮어쓰기* 로도
*뒤에 붙이기* 로도 똑같이 성립한다 — 관측만으로는 두 구현을 못 가른다. 실제로 갈리는 입력은
**같은 이름이 두 번 실린 리스트**뿐인데 그런 자산이 없다.
...
**[해소 2026-08-21] 정렬하지 않는다 — 삽입 순서다.** 컨테이너가 **MSVC `std::list`** 인 것을
순회 코드로 확정했다:

```
0x14016c42d  mov rdi, qword ptr [rdx + 8]   ; 센티널 노드
0x14016c431  mov rbx, qword ptr [rdi]       ; head->_Next = 첫 원소
0x14016c434  cmp rbx, rdi
0x14016c437  je  0x14016c7de                ; 자기 자신이면 빈 리스트 → 종료
0x14016c459  lea r14, [rbx + 0x10]          ; 노드+0x10 = 이름(std::string)
```

`cmp rbx, rdi` 는 **원형 이중연결 리스트의 센티널 비교**다. 노드 배치도 그것과 맞는다 —
`_Next(+0x00)` · `_Prev(+0x08)` · 값(`+0x10` 이름 32B, `+0x30` uint32). `std::map` 이라면
`_Left`/`_Parent`/`_Right` 세 포인터와 `_Isnil`(`+0x19`) 검사가 보여야 하는데 없다
(같은 바이너리에서 실제 `std::map` 순회는 `cmp byte ptr [rax + 0x19], 0` 로 나타난다 —
예: `0x1400f70b6`). **즉 방출 순서 = 리스트에 넣은 순서**이고, 정렬은 없다.
**남는 것**: *누가 어떤 순서로 그 리스트를 채우는가* — 그건 §2.6 의 병합 순서 문제 그대로다.
...
이름 비교는 `0x1401691f5`(`"LightingV1"` = `0x14048be90`). 조각 문자열은
...
**[정정·대부분 해소 2026-08-21]** 위 레이아웃과 꼬리 길이가 둘 다 실측과 다르다. 5파일을
`DXBC` 매직으로 재파스했다(`SHDV0069` 뒤 `u8 0`, 그다음부터 `u32 길이` + `DXBC…`):
...
0x09  uint32 size1      DXBC(정점) 길이
0x0d  DXBC blob1
      uint32 0          **4바이트** (종전 서술의 "8바이트 0x00" 은 이 4바이트 + 다음 길이 필드를
                        같이 센 것이다 — 5/5 전건 4바이트)
      uint32 size2      DXBC(프래그먼트) 길이
      DXBC blob2
      꼬리              **65 · 69 · 69 · 226 · 275 B**(종전 "61~222" 는 범위가 틀렸다)
```

**꼬리는 반사 테이블이 아니라 `usershadervalues` 바인딩 표다.** 꼬리 안에 평문 이름과 기본값이
그대로 들어 있다 — 이건 x86 이 아니라 파일만 보고 확정된다(함정 6).

| 파일 | 엔트리 | 내용(이름, cbuffer 오프셋, 성분 수, 기본값) |
|---|---:|---|
| `011d928b…` | 3 | `bloomstrength` (0x00, 1, `"2"`) · `bloomthreshold` (0x04, 1, `"0.65"`) · `bloomtint` (0x10, 3, `"1 1 1"`) |
| `4889eab2…` | 5 | `Alpha`(0x04,1,`"1"`) · `Bright`(0x00,1,`"1"`) · `Power`(0x08,1,`"1"`) · `Scroll 1 X`(0x00,1,`"0"`) · `Scroll 1 Y`(0x04,1,`"0"`) |
| `1aa4fb0a…` `381e2d24…` `be980a71…` | 0 | 엔트리 없음(꼬리 65~69 B 가 곧 빈 표의 크기) |

엔트리 레코드: `<material 키>\0` · `u32 오프셋` · `u32 성분 수` · `<default 문자열>\0` ·
`<material 키 반복>\0` · 9바이트(0). 엔트리 수는 앞의 `u32`.

**자산이 세 겹으로 검증한다.**
* 이름·기본값 셋이 `assets/shaders/downsample_quarter_bloom.frag:6-8` 의 어노테이션과 **전건 일치**
  (`{"material":"bloomstrength","default":2}` · `{"material":"bloomthreshold","default":0.65}` ·
  `{"material":"bloomtint","default":"1 1 1"}`). 선언 **순서**도 같다.
* 오프셋 `0x00 / 0x04 / 0x10` 이 **HLSL cbuffer 패킹 규칙 그대로**다 — `float`(0) `float`(4)
  다음의 `float3` 는 8 에 놓으면 16바이트 경계를 넘으므로 0x10 으로 밀린다.
* `Scroll 1 X`/`Scroll 1 Y` 처럼 **공백·대문자가 든 저작 키**가 그대로 실린다 — §2.5 의
  "소문자 정규화로 키를 만들면 매칭이 깨진다" 를 캐시 포맷이 독립적으로 증언한다.

**아직 미해결(꼬리 머리 부분).** 엔트리 표 앞에 고정 헤더가 있고 그 안에 엔트리 수만큼의
**6바이트 삼중항**이 들어간다(예: `011d928b` = `8c 00 00 00 00 00` · `8c 00 04 00 01 00` ·
`8e 00 10 00 02 00`). 첫 u16 `0x8c`=140 은 `docs/re/shader-uniforms.md` §3.1b 의
디스패처가 쓰는 "cbuffer 아님" 센티널과 **같은 값**이고 140 은 레지스트리(0..139) 밖이라
"엔진 유니폼이 아니다" 표시로 보이지만 — **연결은 추정이다.** 나머지 필드(`0x0b`·`0x0c`·
`ff` 10바이트 등)의 의미도 못 갈랐다.
...
| 순서 | 내용 | 주소 |
|---|---|---|
| ① | **int32** — 디바이스 가상함수 `[[dev+0x1518]]+0x30` 의 반환(백엔드/셰이더모델 식별자로 보인다, 정체 **[미해결]** — **[미해소 2026-08-21]**, 가상 디스패치라 정적으로는 vtable 후보를 못 좁혔다. 다만 캐시 **디렉터리 이름**이 `blobsSM40`(=Shader Model 4.0)로 고정 문자열이므로, 셰이더 모델은 경로가 이미 가르고 있고 이 int32 는 **그와 다른 축**(디바이스 티어/기능 레벨)일 가능성이 높다 — 추정) | 준비 `0x14016310c`, 해싱 `0x14016ca26` |
...
**[미해소 2026-08-21]** 그대로다. 이 컨테이너에서는 실행 추적을 할 수 없고, 정적으로는
`0x1400d5070` 이 매번 `.dxs` 경로를 조립해 디스크를 두드리는 것까지만 보인다 — 그 앞에
메모리 맵 조회가 있는지는 호출자 사슬을 위로 더 올라가야 하는데 못 올라갔다.
**Waple 에는 영향이 없다** — 우리는 인메모리 메모이즈를 이미 갖고 있고(§8-G5), 실물이
메모리 캐시를 갖든 안 갖든 화면은 같다(성능 축이다).
...
파스 자리는 `0x140154fd0`(같은 머티리얼 패스 파서 안). 동봉 도달 **1건**.

같은 파서가 읽는 형제 키(**VA 는 `lea`/`mov` 의 명령 시작 주소다**. 종전 주석은 "`usertextures`/
`keepaspect` 두 건만 disp32 필드 주소를 적어 어긋나 있었다 — 통일했다" 고 적었는데, **통일이
반대 방향이었다**: 맞던 두 건에 나머지를 맞춘 게 아니라 나머지를 disp32 쪽으로 끌어갔다.
`scripts/re/va_citations.py` 가 잡아 2026-08-21 다시 명령 시작으로 정정했다 — 부록 C 표 참조):
`constantshadervalues` `0x14015464e` ·
`usertextures` `0x140154685` · `usertextures[].keepaspect` `0x140154871` ·
`textures`/`usertexturereference`(`0x1401556e0` 계열) · `shader` `0x140154ce8`.
...
**[미해소 2026-08-21 — 다만 (b) 의 모양이 확정됐다]** 주입은 여전히 안 한다(레인 밖).
새로 아는 것: 그 15개 유니폼(ID 120..134)은 실물에서 **한 상수 버퍼 `g_bufLights`(레지스터 b3)**
에 통째로 들어가고, 그 버퍼 본문은 **고정 텍스트**다(`0x1400f74db` `cbuffer ` + `0x1400f74f2`
`g_bufLights` + `0x1400f751d` `mov byte ptr [rsp+0x44], 0x33` = `'3'`). 즉 (b) 를 구현할 때
"어떤 이름이 몇 개, 어떤 순서로" 는 이미 답이 있다 — `docs/re/shader-uniforms.md` §3.1b ⑤ 의
`g_bufLights` 15행이 그 목록이고, HLSL 쪽 선언 문자열도 `0x140487658`부터 평문으로 박혀 있다
(`const float4 g_LPoint_Origin[` 등). 배열 길이는 `LIGHTS_*` 매크로다.
...
**[해소 2026-08-21] 소수 리터럴도 실물대로 평가한다.** 렉서를 다시 떠서 확정했다 —
`0x140167021 cmp byte ptr [rax], 0x2e` 에서 `.` 을 만나면 `0x140167026 inc rax` 로 **무조건
소비**하고(`isdigit` 검사 **전**이다 — `#if 1.` 도 1), `0x140167031`–`0x140167046` 이 숫자를
읽되 누적기 `esi` 를 **안 건드린다**. 16진 분기도 같은 자리로 합류한다
(`0x140166ff1 jmp 0x14016701e`) — 그래서 `#if 0x10.5` = 16 이다.

**"오역보다 폴터" 를 뒤집는 근거 셋** (`ShaderPreprocessor.weNumericLiteral` 주석에도 적어 뒀다):
1. 실물 동작이 **측정으로 확정**됐다 — 추정이 아니다. "오역보다 폴터" 는 *모르는* 문법에 대한
   규약이고, 아는 문법을 거부하는 것은 이제 **실물과 갈리는 쪽**이다.
2. 회귀 폭 0 — `#if`/`#elif` 식 안의 소수 리터럴도, `#define NAME <비정수 수치>` 를 `#if` 가
   참조하는 자리도 **설치본(`assets/` + `projects/`) 전수에서 0건**(실측).
3. 없으면 워크샵 셰이더의 `#define BLUR 1.5` 한 줄이 **이펙트 전체를 폴백**시킨다.

거부는 그대로 남는다 — 삼항 `?:` · 수 밖의 미지 문자 · **잔여 토큰**(그래서 `#if 1e5` 는
여전히 거부다: 실물도 수를 `1` 에서 끊고 `e5` 를 식별자 토큰으로 내는데, 실물은 잔여를 버리고
우리는 거부한다. §10.9).

회귀 폭: 동봉 등장 0회라 **717 구성의 MSL 지문이 변경 전후 전건 동일**(FNV-1a 비교).
되돌림 실측 7 테스트 케이스 / 31 단정.

**[2026-08-21 독립 확인]** "동봉 등장 0회" 를 다른 방법으로 다시 쟀다 — 동봉 `#if`/`#elif`
**식 322종을 전수로 뽑아** `%` · 비트(`&`/`|`/`^`/`~`) · 시프트 · 16진 · 삼항 · `defined`
사용 건수를 세니 **전부 0건**이었다. 즉 이 확장이 잠복(워크샵 대비 방어)이라는 서술이 맞다.
`defined` 는 그중 유일하게 **실물에도 있는데 우리도 이미 맞았던** 항목이다 — 문자열이 아니라
즉치 비교라(`0x140167ca1` `sub ecx,0x69666564`) 문자열 스캔으로는 "미지원" 으로 오판하기 쉽다.
...
(`GLSLTranslator.uppercasedComboKeys`). 실물의 접기 자리는 JSON 파스(`0x140154598`)지만
...
**[미해소 2026-08-21]** 그대로다 — `Sources/WapleRender/SceneRendererResources.swift` 는 이
라운드의 소유 밖이라 손대지 않았다. 넘길 패치안: `resolvePassCombos` 안의 `canonical()` 호출과
그 헬퍼를 지우고(선언 집합 인자도 함께), 대신 `GLSLTranslator.uppercasedComboKeys` 가 진입에서
접는다는 주석을 남긴다. **삭제만으로 동작이 안 바뀌는 근거**: 번역기 진입에서 무조건 대문자화가
이미 걸리고(§8-G3), 그 앞단에서 한 번 더 접는 것은 멱등이다.
...
* **설치본(`assets/` + `projects/`) 저작 전수: 56건 중 52건이 그림을 바꾼다.**
  ...
  **[원인 확정 · 수정은 미해소 2026-08-21]** "이름이 안 나온다" 가 아니라 **구조적**이다.
  `GLSLTranslator` 가 방출하는 정점 입력은 두 attribute 로 고정이다 —
  `struct VIn { float3 a_Position [[attribute(0)]]; float2 a_TexCoord [[attribute(1)]]; };`.
  `parseAttributes` 는 선언된 이름을 전부 `vin.<이름>` 으로 매핑하므로 그 밖의 attribute 는
  **VIn 에 없는 멤버 참조** → MSL 컴파일 확정 실패 → 폴백이다. 즉 `a_Color` 만의 문제가 아니다.
  설치본 전수 실측(`.vert`/`.h`): `a_Normal` 선언 17파일(**저작레인 8**) · `a_Color` 9(저작레인 1) ·
  `a_BlendIndices`/`a_BlendWeights` 10/9(저작 0) · `a_Tangent4` 8(저작 1) · `a_TexCoordVec4` 계열 등.
  저작레인 8건은 전부 non-preview 기본 프로젝트다 — 목록과 넘길 패치안은
  `docs/re/shader-uniforms.md` §7.6. **이 라운드 미수행**(정점 디스크립터가 `WapleRender` 소관이라
  두 파일이 함께 가야 하고, `GLSLTranslator` 쪽만 고치면 파이프라인 생성이 대신 깨진다).
...
**[2026-08-21 독립 재확인 — 범위 라벨]** 같은 헤더가 한 TU 에 두 번 인라인되는 사례는
**동봉/설치본 502 파일(`.vert` 242 · `.frag` 242 · `.h` 14 · `.geom` 4)에서 0건**이다
(재귀 인라인을 실제로 돌려 전수 실측). 두 트리는 md5 가 전건 동일하므로 같은 모집단이다.
**워크샵 셰이더의 빈도는 모른다** — 잠복 항목이라는 판정은 이 라벨 안에서만 유효하다.
...
* **[착지 2026-08-21]** `inlineIncludes` 에 `inout Set<String> seen` 을 붙여 이름 중복을 스킵한다
  (`ShaderPreprocessor.swift` 의 `inlineIncludes(_:include:depth:seen:)`). 종전 주석은 이 줄을
  "착지" 로 적어 뒀지만 **코드에는 없었다** — `depth > 16` 캡만 있었다. 이번에 실제로 넣었다.
  범위는 실물과 같은 **스테이지 단위**다(실물 `0x140162ee0`/`0x140162f2e` 이 스테이지마다
  컨테이너를 새로 만든다 — Waple 도 `preprocessStrict`/`inlinedSource` 가 스테이지마다 새로 부른다).
  깊이 16 캡은 남긴다(순환은 이제 이름으로 끊기지만, 병렬 분기가 깊게 중첩된 병리적 입력의 스택 방어).
  **회귀 폭 0** — 한 TU 에서 같은 헤더가 두 번 닫히는 자산이 설치본 `.vert`/`.frag`/`.geom`
  전수에서 0건이다(재귀 폐포를 실제로 돌려 재측정). 계약은 `ShaderEngineUniformTypeTests` 의
  `testIncludeIsInlinedOncePerStage` · `testSelfIncludingHeaderTerminatesByNameNotDepth` ·
  `testMemoKeyInlinerSharesIncludeOnce` 가 잡는다.
...
| `"default": 1.5` | `isIntegral` 거짓 → **0** | `jsonInt` 가 `1` 을 뽑는다(`ShaderPreprocessor.jsonInt`) | 자산 도달 0(316/316 정수 또는 부재). **주의**: 이건 `[COMBO]` **JSON** 의 `default` 이고, `#if` 식 안의 소수 리터럴(§8-G2 에서 실물대로 맞춘 것)과 **다른 축**이다 — 여기서는 여전히 갈린다 |
...
1. **콤보 병합 순서**(§2.6) — `[COMBO]` 기본값 / 머티리얼 `combos` / 씬 오버라이드 /
   ...
   → **[미해소 2026-08-21 · 좁아짐]** 컨테이너가 정렬 없는 `std::list` 인 것을 확정했으므로
   "합치는 순서 = 방출 순서" 이고 **텍스트 전처리라 뒤가 이긴다**. 그래서 "덮어쓰기" 구현과
   "뒤에 붙이기" 구현이 **관측상 구별 불가**다 — 갈리는 입력(같은 이름 두 번)이 자산에 없다.
2. **캐시 키 ①의 정수**(§6.2) — `[[dev+0x1518]]+0x30` 가상호출 반환값의 의미.
   백엔드/셰이더모델/디바이스 티어 중 무엇인지 미확정.
   → **[미해소 2026-08-21]** 가상 디스패치라 정적으로 vtable 후보를 못 좁혔다. 다만 캐시
   **디렉터리 이름**이 `blobsSM40`(SM 4.0) 고정이라 셰이더 모델 축은 경로가 이미 가른다 —
   이 정수는 **다른 축**일 가능성이 높다(추정).
3. **`.dxs` 꼬리 61~222 B**(§6.1) — DXBC 두 블롭 뒤에 붙는 메타의 구조.
   → **[대부분 해소 2026-08-21]** 꼬리는 **`usershadervalues` 바인딩 표**다. 엔트리 =
   `<material 키>\0` + `u32 cbuffer 오프셋` + `u32 성분 수` + `<default 문자열>\0` +
   `<키 반복>\0` + 9바이트. 실측 이름·기본값이 `downsample_quarter_bloom.frag:6-8` 의 어노테이션과
   전건 일치하고 오프셋 `0/4/0x10` 이 HLSL cbuffer 패킹 규칙과 일치한다. 길이도 정정 —
   **65·69·69·226·275 B**(종전 "61~222" 는 틀렸다). 블롭 사이 패딩도 8이 아니라 **4바이트**다.
   **남은 것**: 표 앞 고정 헤더의 6바이트 삼중항(`8c 00 …`)과 `0x0b`/`0x0c`/`ff`×10 필드.
4. **`#define` 방출 순서**(§3.2) — 컨테이너 순회 순서가 이름순인지 삽입순인지.
   캐시 키 쪽은 `0x14016e270` 이 만든 목록을 쓰는데 그 함수가 정렬을 하는지 미확인.
   → **[해소 2026-08-21]** **삽입 순서다(정렬 없음).** 방출기가 `cmp rbx, rdi`(원형 이중연결
   리스트 센티널 비교)로 순회하고 노드가 `_Next(+0)`/`_Prev(+8)`/값(`+0x10`) 배치라
   **MSVC `std::list`** 다 — `std::map` 이면 `_Isnil`(`+0x19`) 검사가 보여야 하는데 없다(§3.2).
   캐시 키 쪽 `0x14016e270` 은 **여전히 미확인**이다(별 함수).
5. **프로세스 내 컴파일 중복 회피**(§6.4) — WE 가 같은 SHA-1 을 두 번 만나면
   메모리에서 재사용하는지.
   → **[미해소 2026-08-21]** 정적으로는 `0x1400d5070` 이 매번 `.dxs` 경로를 조립하는 데까지만
   보인다. **Waple 에는 영향 없음**(성능 축, 우리는 이미 인메모리 메모이즈가 있다).
...
8. **[2026-08-21] `#if` 소수 리터럴**(§3.7) — 실물은 `.` 뒤 소수부를 읽고 **버린다**
   ...
   → **[해소 2026-08-21]** 실물대로 맞췄다(§8-G2). 16진 분기도 같은 `.` 검사로 합류하는 것까지
   확인해 `#if 0x10.5` = 16 을 함께 잠갔다. `.` 은 `isdigit` 검사 **전에** 무조건 소비되므로
   `#if 1.` 도 1 이다.
...
python3 "$SP/vdis2.py" 0x140154480 0x140155668   # 머티리얼 패스 파서(대문자화 0x140154598)
```
...
`scripts/re/va_citations.py` 가 이 문서의 인용 336개를 **명령 경계**로 전수 대조해 7건을 잡았다.
여섯 건은 **`disp32` 필드 주소를 명령 주소로 적은 것**이다 — xref 바이트 스캔(`wxref.py`)이
돌려주는 것은 rel32/disp32 **필드**의 위치이지 명령의 시작이 아니다(방법론 함정 16b).
`lea rdx, [rip + d]` 는 `48 8d 15 <d32>` 라 필드가 **+3**, `call rel32` 는 `e8 <d32>` 라 **+1** 밀린다.

옛 주소는 지우지 않는다(툼스톤). 아래 줄들은 `[VA-정정]` 마커가 붙어 검사에서 빠진다.

| 종전 표기 | 정정 | 그 자리의 명령 | 가리키는 곳 | 밀림 |
|---|---|---|---|---|
| `0x140154599` | `0x140154598` | `call 0x1402bfb48` (toupper) | — | +1 (`e8` 뒤 rel32) | <!-- [VA-정정] -->
| `0x140154651` | `0x14015464e` | `lea rdx, [rip + 0x336f7b]` | `0x14048b5d0` `"constantshadervalues"` | +3 | <!-- [VA-정정] -->
| `0x140154cea` | `0x140154ce8` | `mov eax, [rip + 0x3367ce]` | `0x14048b4bc` `"shader"` | +2 | <!-- [VA-정정] -->
| `0x140154fd3` | `0x140154fd0` | `lea rdx, [rip + 0x336639]` | `0x14048b610` `"usershadervalues"` | +3 | <!-- [VA-정정] -->
| `0x1401691f8` | `0x1401691f5` | `lea rdx, [rip + 0x322c94]` | `0x14048be90` `"LightingV1"` | +3 | <!-- [VA-정정] -->
| `0x1401e731c` | `0x1401e7319` | `lea rdx, [rip + 0x2a41a4]` | `0x14048b4c4` `"combos"` | +3 | <!-- [VA-정정] -->

**가리키는 문자열이 전부 서술과 맞았다** — 즉 이 여섯 건은 *어느 명령이냐*만 틀렸고 *무슨 일을
하느냐*는 맞았다. §2.2 의 어셈블리 인용 블록은 원래부터 `0x140154598  call 0x1402bfb48` 로 옳게
적혀 있었고, **본문 산문만 한 바이트 밀려 있었다** — 같은 문서 안에서 블록과 산문이 갈린 형태다.

**검사기가 반대로 판정한 한 건.**
`[VA-정정]` `0x140154599` 를 도구는 "주입본(Ghidra, +0xd0) → 원본 `0x1401544c9`" 로 분류했는데 **그건 틀렸다**. `0x1401544c9` 는 `lea r8, [rip + 0x336ffa]` 로
`toupper` 와 무관하다. `.pdata` 함수 시작에서 선형으로 내려와 직접 확인한 결과가 위 표다
(방법론 함정 15 — 후보 주소에서 거슬러 뜨면 안 된다). **도구 출력도 근거로 삼기 전에 다시 떠라.**

일곱 번째는 §8-G6 이 `ScenePBRLighting.swift` 의 **틀린 인용을 인용**한 것이라 정정 대상이
아니다 — 그 줄은 "이게 왜 틀렸나" 를 적는 자리이므로 값을 그대로 둔다. `[VA-정정]` `0x140168000`

`.text` 안 데이터(점프 표·바이트 색인 표)는 명령이 아니므로 **절대 VA 대신 이미지 베이스 상대
오프셋**으로 적는다 — `docs/re/shader-uniforms.md` §3.1b 의 `베이스+0xf8480`/`베이스+0xf8494` 가
그 규약이다. 검사기가 그 자리를 선형 디스어셈하면 `add byte ptr [rax], al` 같은 쓰레기가 나온다.
