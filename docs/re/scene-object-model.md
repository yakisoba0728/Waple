...
>
> **[2026-08-21 갱신] 이 문서에 남은 `:NNNN` 은 전부 낡았다고 보고 읽어라.** 같은 날 §4.6/§4.7
> 작업이 `SceneDocument.swift` 에 100줄 넘게 넣어서 그 뒤 줄이 전부 밀렸다. 애초에 **다른 파일의
> 줄 번호를 인용하는 것 자체가 이 리포의 금지 관례**다(`docs/dev/re-methodology.md` 함정 22 —
> 한 줄만 밀려도 엉뚱한 곳을 가리키고, 그 상태가 조용히 오래 산다). 새로 적는 자리는 **함수명·
> 심볼명만** 쓴다. 남은 것들을 심볼로 바꾸는 것은 별도 스윕 — **미해결**.
...
| 19 | **중복 `id`** | 중복 제거 **없음**. id 조회(`Scene::findObjectById` `0x140196840`)는 배열 순서 **앞이 이긴다**. 패키지 엔트리의 last-wins 와 반대 | **[2026-08-21 닫음]** 세 부모-맵 빌더가 각자 다른 규칙(레이어 뒤가 이김 · 카테고리 우선)을 들고 있었다 → `SceneDocument.claimObjectID` 하나로 통일 | §4.6 |
| 20 | **부모→자식 상속 축** | 트랜스폼(곱셈) · 가시성(논리 AND) **둘뿐**. alpha/color/brightness 는 상속 안 함 | 일치(`composed()` 가 origin/scale/angle 만 합성) — 이번에 근거를 붙였다 | §4.7 |
| 21 | `sortorder` 정렬 안정성 | **안정(stable)** — 두 경로 다 MSVC `std::stable_sort` | `sortorder` 미파스라 항상 배열 순서 = 이 규약의 특수형(일치) | §9.2b |
| 22 | 커서 히트테스트 참가 타입 | 종류코드 **1(image)·4(text)** 가 같은 2D 쿼드 경로, **5(model)** 이 메시 경로. 나머지 7타입은 참가 안 함. 시차 보정(`parallaxDepth`)은 1·4 **둘 다** 받는다 | 포인터 경로가 아직 텍스트를 대상 목록에 안 넣는다(`WapleRender` 소유 — 넘김) | §12 정정 상자 |
| 23 | camera 의사-오브젝트 `path` | **소비된다** — camera `load` 오버라이드 `0x1401f2030` 이 파일을 읽어 `paths[].transforms[]` 를 쓴다 | 파스·보존만(미소비). 종전 주석의 "인라인 웨이포인트 부재라 미소비" 는 근거가 틀렸다 — 정정 | `SceneCameraObject` 주석 |
...
| `id` | 182 | 292 | 15 | 93 | int | 직접 `0x1401a3922`(헬퍼 `sub_1401a38f0`, 기저 ctor `0x1401ddd69` 이 호출) — 종전 `0x14022b01e` 는 **형제 키**였다(아래 정정) | `SceneDocument.parse` 의 `intVal(obj["id"])` |
...
> **[2026-08-21 정정] `id` 행의 "직접 `0x14022b01e`" 는 `objects[].id` 가 아니었다 — 형제 키다(브리프 함정 8).**
>
> `0x14022b01e` 는 `lea rdx, "id"` 의 disp32 자리이고, 그 명령이 사는 함수(`0x14022af30`–`0x14022b470`)는
> 바로 앞에서 **`json["dependencies"]`** 를 꺼내(`operator[]` `0x14022afb5`, 이름 `lea` `0x14022afae`)
> 태그 6(array)을 확인한 뒤(`0x14022afc0`) 배열 원소를 돌면서 원소의 `id`(`0x14022b029`) ·
> `type`(`0x14022b043`) · `index`(`0x14022b05d`) · `mask`(`0x14022b077`) 를 읽는다. 즉
> **`objects[].dependencies[].id`** 다. `objects[].id` 와 이름만 같다.
>
> 진짜 `objects[].id` 리더는 헬퍼 **`sub_1401a38f0`**(`0x1401a38f0`–`0x1401a39bc`)이고, 오브젝트
> 기저 생성자 `0x1401ddbb0` 이 `0x1401ddd69` 에서 `rcx = &obj->id`(= `obj+8`)로 부른다. 안에서
> `find("id")`(`0x1401a3922`) → 태그 1..3 게이트(`dec eax; cmp eax,2; ja` `0x1401a3935`–`0x1401a393a`)
> → `asUInt64`(`0x140086000`, `0x1401a395f`) → **0 이면 저장 생략**(`0x1401a3964`) →
> `[obj+8] = id`(`0x1401a399e`) → id 집합에 insert(`0x1401a39ab` → `sub_140078250`).
> 그래서 `id` 는 **부호 없는 64비트**이고 `true`/`"3"` 같은 값은 착지하지 않는다.
> 같은 헬퍼를 `instanceoverride` 서브오브젝트도 쓴다(§5.3 의 "id 부여" 가 이것이다).
> 도수 열(F/O/npF/npO)은 코퍼스 측정이라 그대로 유효하다.
...
>   `0x140474c14` 의 begin/end 쌍(`0x140474c14`/`0x140474c1a`)을 쓰는 조회는 `0x140020ee3`
...
> **정확히 2건**(`0x140259455` 의 SSO 복사 · `0x1402594e3` 의 `lea rdx`)이며 **둘 다 텍스트
...
> **[2026-08-21 정정] 위 괄호 안 "10 vtable 오버라이드 0" 은 틀렸다.** 툼스톤으로 남긴다.
> `0x1401850a0` **값을 가진** vtable 만 스캔했으므로 그 값을 안 가진 vtable(= 오버라이드한 타입)은
> 표본에 못 들어온다. 실제로 **image(`0x1404911a8` → `0x1401fd3f0`)와 text(`0x140491950` →
> `0x140256e10`)가 슬롯 `+0x80` 을 오버라이드한다.** 다만 두 오버라이드 모두 `+0x120` 을 읽지 않고
> 기저 `sub_1401850a0` 에 위임한 뒤 타입 고유의 로컬 오프셋만 덧붙이므로 **결론은 그대로다.**
> 타입별 vtable 전수표와 근거는 `docs/re/object-propagation.md` §4 의 정정 상자.

### 4.6 `parent` id 해소 · 중복 id 규약 **[2026-08-21 신규 확정]**

**WE 는 오브젝트를 중복 제거하지 않는다. id 조회는 `objects[]` 배열 순서에서 앞이 이긴다.**

1. **로드는 전건 push.** 씬 로더 `0x140186c90`–`0x140188816` 이 `objects` 배열을 원소 순서로
   순회하고(태그 7 게이트 `0x140187f15`, 팩토리 호출 `0x140187f22`), 만들어진 오브젝트를 씬 벡터
   `[scene+0x158]` 에 push 한다(`0x140190842`). 중복 검사·제거가 **없다** — 같은 id 가 둘이면
   둘 다 존재하고 둘 다 드로우 리스트에 들어간다.
   순회는 std::map(적-흑 트리)의 **중위 순회**다 — 시작이 최좌단(`0x140187ec6`–`0x140187ed4`,
   `_Myhead->_Left` 를 두 번 따라간다)이고 이후가 후행자 걷기(`0x140187f27`–`0x140187f70`,
   `_Isnil` 바이트 `[node+0x19]` 로 판정)다. jsoncpp 는 **배열도** `ObjectValues =
   std::map<CZString, Value>` 로 들고 있고 배열 키의 비교자가 인덱스 순이므로 결과는
   **`objects[]` 파일 순서**와 같다. (트리 걷기는 바이트로 확정, "키 비교자가 인덱스 순" 은
   jsoncpp 구현 사실이고 이 이미지에서 비교자 본체를 따로 뜨지는 않았다.)
2. **id → 오브젝트는 선형 탐색 · 첫 일치.** `Scene::findObjectById` **`0x140196840`**
   (씬 인터페이스 서브오브젝트 vtable `0x14048ea08` 슬롯 `+8`; 그 서브오브젝트 포인터를
   `[ctx+0x1510]` 에 심는 곳이 `0x1401872e8`, 값은 `scene+0x50`)이 유일한 조회 지점이다:
   ```
   0x140196846  rax = [scene+0x158]          ; begin  (서브오브젝트 기준 +0x108)
   0x140196850  r8  = [scene+0x160]          ; end
   0x140196860  rdi = *rax
   0x140196863  cmp [rdi+8], rdx             ; obj->id == id ?
   0x140196867  je  found                    ; ← 첫 일치에서 멈춘다
   0x140196869  rax += 8 ; loop
   0x140196872  return nullptr
   ```
   `found` 뒤에는 "폐기 집합"(`[scene+0x218]/[0x228]/[0x240]` 해시셋) 조회가 한 번 더 있고,
   후보가 그 집합에 들어 있으면 `nullptr` 로 바꾼다(`0x14019694e`–`0x14019696d`).
3. **`parent` 해소도 같은 규칙.** 로드 중에는 `IObject::load` `0x1401de470` 이
   `find("parent")`(`0x1401de4b8`) → 태그 1..3 게이트(`0x1401de4e0`–`0x1401de4e5`) →
   `asUInt64`(`0x140086000`, `0x1401de50c`) 로 읽어 `[obj+0x188]` 에 넣고(`0x1401de51b`)
   `[[obj+0xc8]+0x1510]` 의 `[vt+8]`(= 위 함수)을 부른다(`0x1401de52c`).
   로드가 끝나면 씬 로더의 **2차 패스**가 같은 벡터를 앞에서 뒤로 훑어 자기 자신을 뺀 첫 일치를
   부모로 삼는다(`0x140187fd2` 후보 · `0x140187fd5` 자기 제외 · `0x140187fda` id 비교), 그리고
   후보의 조상 체인에 자기 자신이 있으면 **부모를 지운다**(`0x140187ff7`–`0x140188018`,
   `setParent(this, NULL, 0, -1)` `0x1401de750`).
4. **id 등록부도 first-wins.** 기저 ctor `0x1401ddbb0` 이 `0x1401ddd69` 에서
   `sub_1401a38f0(&obj->id, json)` 를 부른다. 그 함수는 `json["id"]`(`0x1401a3922`)를
   태그 1..3 게이트(`0x1401a3935`–`0x1401a393a`) 뒤 `asUInt64`(`0x1401a395f`)로 읽고,
   **0 이면 저장 자체를 건너뛴다**(`0x1401a3964` `test rax,rax` / `je`) — 즉 `id:0` 은
   `id` 키 없음과 구분되지 않는다. 0 이 아니면 `[obj+8] = id`(`0x1401a399e`) 후
   `sub_140078250` 으로 해시 집합에 넣는데, 그것은 **삽입(insert)** 이라 같은 키가 이미 있으면
   기존 노드를 돌려주고(`0x14007831f` `xor al,al` = inserted=false) **덮어쓰지 않는다.**

> **형제 이름 주의(브리프 함정 8).** 패키지 엔트리 색인은 커밋 `adda85e` 가 확정한 대로
> **뒤가 이긴다(last-wins)** 다(`Sources/WapleCore/ScenePackage.swift` 선언 주석).
> 오브젝트 id 는 **그 반대**다. 이름이 같은 다른 코드이므로 한쪽 결론을 다른 쪽에 옮기지 마라.

**동봉/설치본 도달: 0.** 씬 문서 186개 / 오브젝트 294개에 중복 id **0건**,
`id` 키 부재 2건, `id == 0` 0건. 즉 이 규약은 워크샵 씬에서만 갈린다.

**Waple**: 종전에는 세 부모-맵 빌더가 각자 다른 규칙을 들고 있었다 — 레이어끼리는 **뒤가** 이기고
(`localT[l.id] = …` 무조건 대입), 레이어↔노드↔텍스트는 배열 위치와 무관하게 카테고리 우선순위
(레이어 > 노드 > 텍스트)로 갈렸다. 2026-08-21 에 술어 하나
(`SceneDocument.claimObjectID`)로 합치고 `objects[]` 순서 first-wins 로 맞췄다.
같은 라운드에 `composeParentTransforms` 가 자기 트랜스폼을 `world(자기 id)` 로 우회해서 읽던 것도
**자기 로컬 × 부모 월드** 직접 합성으로 바꿨다 — 우회 형태는 중복 id 에서 **진 쪽 오브젝트를 승자
좌표로 끌어다 놓는다**(WE 는 오브젝트마다 자기 로컬과 자기 부모 포인터를 들고 있어 그런 뒤섞임이
없다). 형제 셋(라이트·텍스트·파티클)은 원래 직접 형태였으므로 이제 네 곳이 같은 모양이다.
**조상 해소는 여전히 id 키맵을 거치는데 그게 맞다** — WE 의 `findObjectById` 도 승자를 돌려준다.

**의도적 이탈 1건(도달 0)**: `parent: 0`. WE 에서는 `id` 키가 없는 오브젝트도 id 0 이라
`parent:0` 이 로드 시점(`0x1401de52c`)에 "배열 앞쪽의 id 0 인 첫 오브젝트" 로 붙을 수 있고
(2차 패스는 `parent id == 0` 을 건너뛴다 — `0x140187fbd`), 즉 **로드 순서에 의존하는 기벽**이다.
설치본 186 씬에서 `parent` 값은 `24` 2건뿐이고 `parent:0`·`id:0` 은 0건이라 실물이 없다.
Waple 은 "부모 없음" 으로 고정한다.

### 4.7 부모→자식으로 물려받는 속성 **[2026-08-21 신규 확정]**

| 축 | 상속되는가 | 규약 | 근거(VA) |
|---|---|---|---|
| 트랜스폼(4×4) | **예** | **곱셈** `World = Local · ParentWorld`. 게이트는 `parent != null` 하나 | `sub_1401850a0` — `0x14018528a` parent 로드 · `0x140185294` 부모 없으면 로컬 반환 · `0x1401852b0` 재귀 · `0x1401852c0` 행렬곱(§4.4) |
| 가시성 | **예** | **논리 AND**(곱셈 아님). 자기 bit0 ∧ 조상 전건 bit0 | `sub_140185010` — `0x140185014` `test byte [rcx+0x120], 1` → `0x14018501d` `rcx = [rcx+0x180]` → `0x140185029` 자기 재귀 |
| 투명도 `alpha`(`+0x33c`) | **아니오** | 자기 값만 쓴다 | 아래 |
| 색 `color`(`+0x330`) · 밝기 `brightness`(`+0x340`) | **아니오** | 자기 값만 쓴다 | 아래 |

셋의 부재 근거: `.pdata` 함수 전수를 디스어셈블해 `+0x330`/`+0x33c`/`+0x340` 을 접근하는 함수를
모두 뽑고, 그중 부모 포인터 `[obj+0x180]` 을 **함께** 역참조하는 것을 셌다 → **0개**.
(`+0x33c` 접근 함수는 16개인데 `+0x180` 도 나오는 것은 `0x1401891a0` 하나뿐이고, 거기서
`+0x33c` 는 **카메라 경로 레코드**의 fov(`0x1401893f0`, 대안이 `[cam+0x2d8]`)이고 `+0x180` 은 위
가시성 AND 호출(`0x140189232`–`0x14018923e`)이다 — 같은 구조체가 아니다.)
즉 **부모의 알파/색은 자식에 곱해지지 않는다.**

**적용 순서.** WE 는 **자식이 요구할 때** 부모 월드를 재귀로 만든다(pull, 프레임 스탬프 캐시
`sub_140185040`). 그래서 "부모 먼저 계산" 같은 전역 순서가 없다. Waple 은 파스 말미에 같은
재귀를 한 번 돌려 굽는다(push) — 결과는 같고, 사이클은 `depth < 32` 로 끊는다(WE 는 §4.6 ③의
2차 패스에서 사이클이면 부모를 아예 지운다).
...
> **[해소 2026-08-21]** 이미터의 **방향 기저**다 — CP 갱신부 `0x14022e3e0` 이 `+0x80` 4×4 를
> 레코드 앞 `0x40`(활성 트랜스폼)으로 복사하고(`0x14022e47b`–`0x14022e4ae`, 게이트는
> `[cp+0xc0]` bit0 set · bit16 clear), 이미터 형상 평가부 `0x1402378a0` 이 그것을 기저 3행 +
> 위치 행으로 읽는다(`0x140237c33` 이후). 전문은 §14.
...
### 9.2b 정렬은 **안정(stable)** 이다 **[2026-08-21 확정 — §14 의 [미해결] 해소]**

두 경로 다 MSVC `std::stable_sort` 의 인라인 형태다.

* **원소 ≤ 0x20**: `0x14019fde0`–`0x14019feb9` 는 `_Insertion_sort_unchecked` 다.
  바깥 루프가 `Next = First+1 … Last`(`0x14019fe7d` `add rdi,8` · `0x14019fe84` `jne`),
  안쪽은 `Val` 을 앞으로 밀되 **비교자가 참인 동안만** 민다
  (`0x14019fe74` `call rbp` → `0x14019fe76` `test al,al` → `0x14019fe78` `jne` 로 계속).
  비교자는 **엄격 `<`**(`setl`)이므로 **같은 키에서 멈춘다** = 같은 값의 원래 순서가 보존된다.
  `Val` 이 맨 앞보다 작을 때만 `memmove`(`0x1404210f0`, `0x14019fe40`)로 블록 이동한다.
* **원소 > 0x20**: 드로우 리스트 빌더가 `ceil(n/2)` 원소짜리 임시 버퍼를 잡는다 —
  `rbx = n - n/2`(`0x14018ab94`–`0x14018ab9f`), `<= 0x200` 이면 스택 버퍼로 가고
  (`0x14018aba9`), 아니면 `0x1402ecff0` 로 `rbx*8` 바이트를 잡되 실패하면 **크기를 반으로 줄여
  재시도**한다(`0x14018abfd` `shr rbx,1` → `0x14018ac00` `jne` 루프). 이게 MSVC
  `_Optimistic_temporary_buffer` 의 서명이다. 그 버퍼와 비교자 `0x140186980` 을 들고
  `0x14019fec0` 를 부른다(`0x14018ac3d`).
* `0x14019fec0`–`0x1401a0d64` 가 `_Stable_sort_unchecked` 다: `_ISORT_MAX = 0x20` 비교
  (`0x14019feeb` · `0x1401a0087` · `0x1401a00db` …), 청크는 `0x14019fde0`(위 삽입정렬)로
  (`0x1401a009c` · `0x1401a0283`), 그 위는 **자기 자신을 두 번 재귀**(`0x1401a02a2` · `0x1401a02c5`)한 뒤
  병합(`0x1401a19e0` · `0x1401a2b40` · `0x1401a32e0`)한다.

→ **같은 `sortorder` 값끼리는 `objects[]` 배열 순서가 유지된다.** Waple 이 `order` 로 안정 정렬을
하면 일치한다(현재 Waple 은 `sortorder` 를 파스하지 않으므로 항상 배열 순서 = 이 규약의 특수형).
...
| 11 | `sortorder` 미파스 | `SceneDocument.parse` 전역 | 디스크립터 `+0x124`, **타입코드 0 = int32**(§1.3 정정 · `object-propagation.md`), 비교자 `0x140186980`, 정렬은 **안정**(§9.2b) | 0 (그러나 워크샵 씬은 쓸 수 있다) |
| 12 | 커서 히트테스트 대상에 텍스트가 없다 | `SceneRenderer.hoverParallaxShift` / `hoverTargets` (**`WapleRender` 소유 — 넘김**) | 종류코드 1(image)·4(text)이 같은 갈래(`0x14018a044`·`0x14018a04b`), 시차 보정도 둘 다(`0x14018a0ff`·`0x14018a10d`) | 텍스트 5 오브젝트 |
| 13 | camera 의사-오브젝트 `path` 미소비 | `SceneDocument.parseCameraObject` 의 `cam.path` (보존만) · `parseCamera` 는 `scene.camera.paths` 를 아예 안 읽는다 | camera `load` 오버라이드 `0x1401f2030` 이 파일을 읽어 `paths[].transforms[]` 소비 | `objects[]` 0 / `scene.camera.paths` **4씬** |
...
호버 히트테스트(`SceneRenderer.hoverParallaxShift`)도 이미지 레이어만 시차 보정을
하고 텍스트는 대상 목록에 없다 — 텍스트에 포인터 스코프가 생기면 같이 봐야 한다. **[미해결]**

> **[해소 2026-08-21] 답은 "텍스트도 이미지와 똑같이 본다" 다.** (위 줄은 툼스톤으로 남긴다.
> 그리고 종전 문면이 인용하던 `:553` 은 **다른 파일의 줄 번호**라 브리프 함정 22 위반이었다 —
> 줄 번호를 지우고 심볼명만 남긴다.)
>
> 커서 이벤트 디스패처 `sub_140189e10`(`0x140189e10`–`0x14018aab9`)는 오브젝트를 뒤에서 앞으로
> 돌면서(`0x14018a404`) 먼저 `solid`(bit13) 게이트를 걸고(`0x14018a02d`), 그다음 **오브젝트
> 종류코드**를 가상 함수로 읽어(`0x14018a041` `call [rax+0x60]`) 히트테스트 대상을 고른다:
>
> ```
> 0x14018a044  cmp eax, 1 ; je  →  rdx = obj      ; 1 = image
> 0x14018a04b  cmp eax, 4 ; jne skip → rdx = obj  ; 4 = text
> 0x14018a058  cmp eax, 5 ; cmovne rdi, 0         ; 5 = model
> 0x14018a067  두 포인터가 다 0 이면 이 오브젝트는 건너뛴다
> ```
>
> 종류코드는 타입별 vtable 슬롯 `+0x60` 이 돌려주는 상수다 —
> **image = 1**(`0x140050790`) · particle = 2 · sprite = 3 · **text = 4**(`0x1400fde90`) ·
> model = 5 · light = 6 · sound = 7 · camera = 8 · shape = 10 · 기저/노드 = 0.
> 즉 **image 와 text 가 같은 갈래**로 2D 쿼드 히트테스트 `sub_14019dbb0`(`0x14018a242`)에 가고,
> model 만 `sub_140185520`(`0x14018a265`)으로 간다. 나머지 타입은 히트테스트를 아예 안 받는다
> (덧붙여 `[vtbl+0x88]` 은 `0x14018a2bb` 에서 불리는 **타입별 2차 판정** 슬롯인데, 실구현이
> 있는 것은 **image `0x1401fd690`** 와 **model `0x140223810`** 둘뿐이고 나머지 8타입은
> `xor al,al; ret` 스텁 `0x1400a36d0` 이다.)
>
> **시차 보정도 같은 갈래에서 함께 일어난다.** `rdx`(= image 또는 text)가 있을 때만 도는
> `0x14018a0b3`–`0x14018a115` 가
> `off = amount(`[scene+0x334]`) * (obj.origin(`[obj+0x128/0x12c]`) − camOffset(`[scene+0x340/0x344]`))`
> 를 만든 뒤 **`obj.parallaxDepth`(`[obj+0x170]`/`[obj+0x174]`)를 성분별로 곱해**
> `[rsp+0x40..0x48]` 에 넣고, 그 3원소를 히트테스트에 `r8` 로 넘긴다(`0x14018a235`).
> 즉 **텍스트의 호버 판정에도 `parallaxDepth` 가 그대로 곱해진다** — 이미지와 다른 규칙이 아니다.
>
> **넘길 것(소유 밖 — `Sources/WapleRender/SceneRenderer.swift`).** 텍스트에 포인터 스코프를
> 붙일 때 `hoverParallaxShift` 대상 목록에 텍스트를 **같이** 넣어야 한다. 새 규칙을 만들지 말고
> 이미지와 **같은 술어**를 쓰면 된다(중복 술어 금지 — `83b807f` 이 지운 네 번째 사본과 같은 사고다).
...
* **[해소 2026-08-21] CP 회전의 최종 소비자 = 이미터의 방향 기저다.** 아래 옛 문면은 툼스톤이다.
  경로는 두 단계다.
  1. **활성 트랜스폼으로 복사.** CP 갱신부 `0x14022e3e0` 이 `rsi = [sys+0x400]`,
     `imul rdi, i, 0xd0`(`0x14022e45a`)로 레코드를 잡고, 플래그워드 `[cp+0xc0]` 를 읽어
     **bit16 이 서 있으면 다른 경로로 빠지고**(`0x14022e468` `bt edx,0x10` → `jb`),
     **bit0 이 꺼져 있으면 건너뛴다**(`0x14022e472` `test dl,1` → `je`). 통과하면
     `+0x80`/`+0x90`/`+0xa0`/`+0xb0` 4행을 그대로 `+0x00`/`+0x10`/`+0x20`/`+0x30` 에 복사한다
     (`0x14022e47b`–`0x14022e4ae`). 즉 레코드 앞 `0x40` 바이트가 **CP 의 활성 4×4** 다.
     (게이트 비트가 §6.4 의 `instanceoverride` CP 게이트 `flags & 0x10005` 와 **같은 워드·같은 비트**다.)
  2. **이미터가 그것을 기저로 읽는다.** 이미터 형상 평가부 `0x1402378a0`
     (호출부 `0x14023771b`, 그 함수는 `0x140236cd0`)이 `rax = [rsi+0x400]`,
     `edx = [emitter+0xf0]`(CP 인덱스), `imul rcx, rdx, 0xd0`(`0x140237c33`) 뒤
     `+0x00`/`+0x04`/`+0x08`(행0) · `+0x10`(행1) · `+0x20`(행2) · `+0x30`(행3=위치)을 읽고,
     **위치 행에 이미터 로컬 오프셋 `[emitter+0x70]` 을 더한다**(`0x140237c65` `addps`).
     이미터 종류 분기(`0x140237c18` `cmp al,1`)의 **양쪽** 갈래가 같은 레코드를 읽는다
     (다른 갈래는 `0x140238487`–`0x1402384d8`, CP 인덱스가 `[emitter+0xa8]`).
  → 그러므로 **`directiontocontrolpoint` remap 이 아니다.** 그 문자열(`0x140491f48`)은 코드에서
  직접 참조되지 않고 값 소스 이름 표 `0x140484ed0`–`0x140484f38`(14엔트리)의 한 칸일 뿐이며,
  같은 표의 `controlpoint`·`deltatocontrolpoint` 와 함께 **위치 계열**이다.
...
* ~~**[미해결] CP 회전의 최종 소비자.** 런타임 CP 레코드(`[sys+0x400] + i·0xD0`)의 `+0x80` 4×4 가
  ...
  **[2026-08-21 좁힘]** `directiontocontrolpoint` 문자열(`0x140491f48`)은 **코드에서 직접
  참조되지 않는다** — .rdata 의 이름 표 `0x140484ed0`–`0x140484f38`(14엔트리: `timeofday` ·
  `particlesystemtime` · `layertime` · `color` · `position` · `velocity` · `controlpoint` ·
  `deltatocontrolpoint` · `directiontocontrolpoint` · `layerorigin` · `remap` · `multiply` ·
  `add` · `subtract`)의 한 칸일 뿐이다. 즉 그것은 **값 소스 id** 이고 CP 의 *위치* 계열이다.
  회전(`+0x80` 행 3개)을 읽는 후보는 CP 스트라이드 `imul …, 0xd0` 과 `[X+0x80]` 을 **함께** 갖는
  8개 함수로 좁혔다 — `0x14022a070` · `0x14022bd40`(생성부) · `0x14022c3c0` · `0x14022e3e0` ·
  `0x140236cd0` · `0x1402378a0` · `0x14023b340` · `0x14023fbc0`. 그중 회전 3행을 **전부** 읽는 것은
  `0x14022c3c0` · `0x1402378a0` · `0x14023b340` 이다. 이 셋이 CP 레코드를 보는 것인지 다른
  구조체의 같은 오프셋인지는 확인하지 않았다 — **여전히 미해결**.
...
* **[해소 2026-08-21] `sortorder` 정렬의 안정성 — 두 경로 다 안정(stable)이다.**
  `0x14019fde0` 는 엄격 `<` 로 미는 삽입정렬이고, `0x14018ab91` 쪽은 `ceil(n/2)` 임시 버퍼를
  잡아(`0x14018ab94`–`0x14018ac00`) `_Stable_sort_unchecked` `0x14019fec0` 로 들어간다.
  근거 전문은 §9.2b. 동봉 도달이 0 이라 **실측 대조는 여전히 불가능**하고, 결론은 바이너리 단독이다.
...
* **[미해결 · 신규] 프로퍼티 디스크립터 타입코드 `3`.** `0` = int32 · `1` = vec2 · `2` = vec3 ·
  `4` = float · `5` = string · `6` = bit 접근자(bool)까지는 등록 사례로 확정했다
  (`docs/re/object-propagation.md` §1.3 정정 상자). `3` 은 이 이미지에서 등록 사례를 못 찾았다 —
  vec4 로 **추정**하나 근거가 없다.
* **[미해결 · 신규] 기저 프로퍼티 색인이 파생 클래스 색인으로 복사되는 시점.**
  조회(`[vtbl+0x20]`)와 열거(`[vtbl+0x10]`)는 **자기 클래스 색인만** 찌르므로 파생 색인이 이미
  기저 항목을 담고 있어야 하는데(반증법으로 확정 — `object-propagation.md` §11 ⑤),
  등록 헬퍼(`sub_14015a000`/`sub_140178e90`)가 받는 벡터
  (`0x1404e8250`/`0x1404e8290` 기저 · `0x1404e8360` image)에서 해시 색인
  (`0x1404e82d8`대 기저 · `0x1404e85e8`대 image)이 만들어지는 자리는 뜨지 않았다.
...
```bash
python3 $S/vdis2.py 0x14022be9c 0x14022c0d0   # FLT_MAX 센티널 · sin/cos · 4×4 · flags 게이트
...
python3 $S/vdis2.py 0x14018ac5c 0x14018acc0   # and 0x1008/cmp 0x1000 (transparentsorting)
```

---

> **[2026-08-21 · VA 인용 정정 5건]** `scripts/re/va_citations.py` 로 이 문서의 VA 를 전수
> 대조해(`wallpaper64.exe` `.pdata` 조각 시작에서 선형 디스어셈) **명령 경계가 아닌 인용 5건**을
> 고쳤다. 종전 주소에서 디스어셈하면 경계가 어긋나 **없는 명령이 보인다**(방법론 함정 15).
>
> | 종전 | 정정 | 실제 명령 | 왜 밀렸나 |
> |---|---|---|---|
...
>
> 앞의 셋은 전부 같은 부류다 — **xref 바이트 스캔이 주는 것은 명령 주소가 아니라 `disp32` 가
> 박힌 자리**이고, 그걸 그대로 적으면 정확히 3바이트 밀린다(방법론 함정 16b).
> 뒤의 둘은 `vdis2.py <시작> <끝>` 재현 명령의 **시작**이라 반드시 경계여야 한다(끝은 배타적이라
> 경계가 아니어도 된다 — 그래서 이 문서의 다른 범위 끝은 손대지 않았다).
>
> **`Sources/WapleCore/SceneDocument.swift` 의 주석에도 `0x140259458`/`0x1402594e6` 이 그대로
> 남아 있다** — 그 파일은 이 라운드에 다른 작업자가 쥐고 있어 여기서 안 고쳤다.
