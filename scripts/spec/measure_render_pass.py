...
def main():
    ...
    entries = [
        ...
        specfmt.entry("engine.renderPass.addressSemantics",
                      {"규약선언": "**[2026-08-28] 이 문서의 `at` 은 명령 시작 주소가 아니라 `disp32` "
                                    "필드 위치다.** 리포의 다른 정본 문서는 `at`/주소를 **100% 명령 시작**으로 "
                                    "쓴다 — 이 문서 하나만 규약이 다르다. 그 사실이 어디에도 선언돼 있지 않아 "
                                    "인용을 그대로 디스어셈블한 사람이 반복해서 경계 밖에 착지했다"
                                    "(`scripts/re/va_citations.py` 전수 대조가 이 파일에서만 113건을 뽑았다). "
                                    "규약이 다르면 **문서가 스스로 그렇게 말해야 한다.**",
                       "자기정합": "규약의 검산은 문서 안에 있다 — `symbolRefs` 의 모든 참조에 대해 "
                                    "**`at + 4 + disp32 == 형제 stringVA`** 가 성립한다(**77/77**). "
                                    "`+4` 는 RIP 상대 주소가 **disp32 필드의 끝** 기준이기 때문이다. "
                                    "이 항등식이 성립한다는 것 자체가 `at` 이 명령 시작이 아님을 증명한다.",
                       ...
                       "symbolRefs.refs[].fn": "**`.pdata` 조각 시작이 아니라 체인 루트다.** `fn` 을 "
                                               "`.pdata` RUNTIME_FUNCTION 의 `BeginAddress` 와 그대로 대조하면 "
                                               "**48/77 만 맞는다** — 나머지 29건은 함수가 여러 조각으로 "
                                               "쪼개져 있어 조각 시작을 가리키기 때문이다. "
                                               "`UNW_FLAG_CHAININFO`(unwind info 플래그 비트 2)를 따라 "
                                               "루트까지 올라가면 **77/77 정확**하다. 생성기의 "
                                               "`chain_root()` 가 그 일을 한다 — `fn` 을 재현하려면 같은 "
                                               "언와인드 추적을 해야 하고, 단순 `.pdata` 이분탐색으로는 "
                                               "재현되지 않는다.",
