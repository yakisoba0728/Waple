...
def main():
    ...
    entries = [
        ...
        specfmt.entry("oracle.nondet.cause", {
            "status": "원인 미상 — 마운트/로드 시점까지는 좁혔다(oracle.nondet.divergesAtMount)",
            "candidates": ["텍스처 디코드·업로드 결과(특히 저장 mip 체인 — 불안정 29종이 전부 그 영향권)",
                           "시스템/GPU 메모리 레이아웃", "드라이버·파이프라인 캐시류"],
            "whyNotScriptsOrSim": "t=0 발산이라 파티클/스크립트 누적은 원인이 아니다. "
                                  "로드 로그도 두 상태가 완전히 같다.",
            "nextStep": "게이트 매트릭스(WAPLE_BC_NATIVE=0 · WAPLE_DISABLE_TRANSLATED=1 · "
                        "WAPLE_LAYER_TRUNC=n · WAPLE_NO_BLOOM)를 상태마다 떠서 "
                        "**상태 간에도 동일해지는 설정**을 찾는다. 그 설정이 끄는 경로가 원인이다.",
            "captureCaveat": "다중 시각 마운트의 t=6 프레임은 t=6 단독 캡처와 다르다"
                             "(캡처 루프가 프레임 간 스크립트/파티클 연속성을 유지 — 실측). "
                             "진단 프레임은 같은 WAPLE_CAPTURE_TIME 끼리만 비교할 것.",
        }, "추정", [ev_probe, specfmt.ev("script", "scripts/mac-session/capture-tsweep.sh")]),
    ]
