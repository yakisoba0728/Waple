...
#
# **[정정 2026-09-01] 이 절의 세 문장이 전부 같은 파일 위쪽과 형제 스크립트에 반박됐다.**
# 종전 서술은 ① "기본 락 파일은 작업 디렉터리의 **상위 + `/swift.lock`**", ② 그래서
# `WAPLE_LINUX_TYPECHECK_DIR` 을 공유 디렉터리로 잡으면 락이 자동으로 묶인다,
# ③ "`linux-core-tests.sh` 는 스스로 락을 잡지 않으므로 호출자가 감싼다" 였다. 셋 다 틀렸다:
#   ① 실제 `LOCK` 정의(위 `LOCK="${WAPLE_SWIFT_LOCK:-${TMPDIR:-/tmp}/waple-swift.lock}"`)는
#      `WORK` 와 무관한 **고정 경로**다.
#   ② 그 유도 방식은 위 정의 바로 앞 주석이 **명시적으로 폐기**했다 — 디렉터리가 다른 두
#      실행이 서로 다른 락을 잡아 상호배제가 아예 안 됐다(실측 2026-08-21: `/proc/locks` 에
#      `/tmp/swift.lock` 과 `<스크래치패드>/swift.lock` 이 동시에 잡혀 있었다).
#      그래서 지금은 **유도에 기대지 말라**가 규약이다.
#   ③ `linux-core-tests.sh` 는 지금 **스스로 락을 잡는다** — 같은 날 같은 이유로 고쳤고
#      (그 파일의 「공유 락 (2026-08-21)」 절), 이 스크립트와 **같은 고정 경로**로 수렴한다.
#
# 정리: 두 스크립트의 기본 락은 `${TMPDIR:-/tmp}/waple-swift.lock` 하나다. 다른 swift 작업과
# 묶거나 다른 락을 쓰려면 `WAPLE_SWIFT_LOCK` 을 **명시**해라.
