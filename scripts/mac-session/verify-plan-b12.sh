...
echo "  번들 합: ${TESTS:-?}  (기준값 2,266 — AGENTS.md, 2026-08-17 실측. 코퍼스 유무와 무관하다)"
echo "  번들별:"
printf '%s\n' "$BUNDLE_SUMMARY" | sed 's/^/    /'
...
# ba64c99·88c195e 이후 `spec/golden/snapshot/baseline-9a5f71a/`(170종, **release**,
...
#   WAPLE_PRE_RELEASE_BASELINE=spec/golden/snapshot/baseline-9a5f71a
...
if [ -z "$PRE" ] || [ ! -d "$PRE/thumbs" ]; then
    ...
    echo "     차선책: 커밋된 release 기준선 spec/golden/snapshot/baseline-9a5f71a 를"
