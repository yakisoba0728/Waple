...
#
# [2026-08-01 심야] 이 스크립트의 결과는 이제 해석이 끝났다 — 같은 세션 안의 두 캡처는
# 항상 0종이 나온다(그래서 여기서 0 이 나와도 "결정적" 이 아니다). 변동은 **세션 간**에만
# 나타난다: spec/golden/nondeterminism.json. 축을 다시 재려면
# scripts/mac-session/probe-session-nondeterminism.sh 를 쓸 것.
# 주의: 아래 루프가 한 번 호출에 runA·runB 를 **둘 다** 뜬다 — 두 캡처 사이에 부하를 끼우는
# 실험은 이 스크립트로는 성립하지 않는다.
#
