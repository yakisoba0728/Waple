...
final class PuppetPoseWEParityTests: XCTestCase {
    ...
    // MARK: - 키 각 3축의 파일 순서 (Z, Y, X)

    /// MDL 로더는 36B 키의 +0x0c/+0x10/+0x14 를 각각 Z/Y/X 반각으로 sin·cos 해서 쿼터니언을 굽는다
    /// (0x140264188–0x1402642ae, 반각 상수 0.5f @ 0x1404926c0). 즉 **첫 슬롯이 Z** 다.
    /// 종전 Waple 은 이 셋을 (x,y,z) 로 읽어 X 축과 Z 축이 뒤바뀌어 있었다.
    func testKeyAngleTripleIsFileOrderZYX() {
        let t: Float = .pi / 2
        // 첫 슬롯 = Z(yaw): +x → +y
        let qz = PuppetPose.rotationQuaternion(SIMD3(t, 0, 0))
        let vz = rotate(qz, SIMD3(1, 0, 0))
        XCTAssertEqual(vz.x, 0, accuracy: 1e-5)
        XCTAssertEqual(vz.y, 1, accuracy: 1e-5, "파일 첫 각 슬롯(+0x0c)은 Z 회전")
        ...
        // 마지막 슬롯 = X(roll): +y → +z
        let qx = PuppetPose.rotationQuaternion(SIMD3(0, 0, t))
        let vx = rotate(qx, SIMD3(0, 1, 0))
        XCTAssertEqual(vx.x, 0, accuracy: 1e-5)
        XCTAssertEqual(vx.y, 0, accuracy: 1e-5)
        XCTAssertEqual(vx.z, 1, accuracy: 1e-5, "파일 세 번째 각 슬롯(+0x14)은 X 회전")
    }

    /// 굽힌 쿼터니언의 행렬 == `Rz(a[0])·Ry(a[1])·Rx(a[2])`. (헥스 식과 수치 항등 확인용 고정점)
    func testRotationQuaternionEqualsRzRyRxOfFileOrder() {
        ...
        let a = SIMD3<Float>(0.3, -0.7, 1.1)   // 파일 순서 (+0x0c, +0x10, +0x14)
        let fromQuat = PuppetPose.quaternionMatrix(PuppetPose.rotationQuaternion(a))
        let expected = rz(a.x) * ry(a.y) * rx(a.z)
        for c in 0..<3 {
            for r in 0..<3 {
    ...
    func testKeyInterpolationTakesShortestArcNotEulerLerp() {
        ...
        let k0 = PuppetModel.Key(position: .zero, angles: SIMD3(170 * d2r, 0, 0), scale: SIMD3(1, 1, 1))
        let k1 = PuppetModel.Key(position: .zero, angles: SIMD3(-170 * d2r, 0, 0), scale: SIMD3(1, 1, 1))
    ...
    func testNlerpEndpointsAreExactRotations() {
        let d2r: Float = .pi / 180
        let a = PuppetPose.rotationQuaternion(SIMD3(30 * d2r, 0, 0))
        let b = PuppetPose.rotationQuaternion(SIMD3(-160 * d2r, 0, 0))
    ...
    func testSlerpBaselineThresholdAndEndpoints() {
        ...
        let d2r: Float = .pi / 180
        let a = PuppetPose.rotationQuaternion(SIMD3(0, 0, 0))
        let b = PuppetPose.rotationQuaternion(SIMD3(90 * d2r, 0, 0))
        // 끝점은 nlerp 와 동일.
        ...
        let c = PuppetPose.rotationQuaternion(SIMD3(0.0002, 0, 0))
    ...
    func testWeightedCascadeRotationUsesNlerpNotMatrixLerp() {
        ...
        let k90 = PuppetModel.Key(position: .zero, angles: SIMD3(.pi / 2, 0, 0), scale: SIMD3(1, 1, 1))
