...
/// **[2026-08-21 정정]** 등록 주소 둘은 종전 `0x1401e06a6`/`0x1401e075c` 였다 — xref 스캔이 준  [VA-정정]
/// `disp32` 위치라 3바이트 밀려 있었다(가리키는 대상은 그대로 `"scale"` `0x14048f64c` ·
/// `"angles"` `0x14048fc5c`). `scripts/re/va_citations.py` 전수 대조로 잡았다.
///
/// | 2 | `SceneLayer.scaleZ` — `scale` 은 vec3 디스크립터 | 등록 `0x1401e06a3`("scale") → `+0x134`(`0x1401e06c6`) → 태그 **2**(`0x1401e06ea`) | 이미지 4 / 21 · 텍스트 3 / 5 |
/// | 3 | `angleX`/`angleY` — `angles` 도 vec3 | 등록 `0x1401e0759` → `+0x140`(`0x1401e0795`) → 태그 **2**(`0x1401e07ae`) | 레이어·텍스트 **0 / 0** |
