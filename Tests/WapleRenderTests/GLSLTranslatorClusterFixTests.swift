...
final class GLSLTranslatorClusterFixTests: XCTestCase {
    ...
    /// F396: 종전엔 Metal 부재 시 그냥 return 해 XCTest 가 "스킵"이 아니라 "어서션 0개로 통과"로
    /// 집계했다(주석은 "스킵"이라 주장했지만 실제로는 아니었음) — 이 헬퍼에 의존하는 7개 클러스터
    /// 회귀 테스트 전부가 비-Metal 환경에서 green 으로 오분류될 수 있었다. throws 로 바꿔 진짜
    /// XCTSkip 을 던진다(동일 레인의 다른 파일들과 관례 통일).
    ...
    func testHashIfNoSpaceBeforeParen() throws {
        let frag = """
        ...
        """
        try assertCompiles(vertex: trivialVert, fragment: frag, combos: ["GRIDX": 1])
    }
    ...
    func testRadiansAndDegrees() throws {
        let frag = """
        uniform sampler2D g_Texture0;
        ...
        """
        try assertCompiles(vertex: trivialVert, fragment: frag)
    }
    ...
    func testRoundResultKnownSizeTruncatesMul() throws {
        let frag = """
    ...
    func testMixMismatchedVectorArgs() throws {
        let frag = """
        uniform sampler2D g_Texture0;
        varying vec2 v_TexCoord;
        void main() {
    ...
    func testMaxIntParamMixedWithFloat() throws {
        let frag = """
        uniform sampler2D g_Texture0;
        varying vec2 v_TexCoord;
    ...
    func testArrayCopyInitAndFloatIndex() throws {
        let frag = """
        uniform sampler2D g_Texture0;
    ...
    func testGlobalConstWithRuntimeCallDemoted() throws {
        let frag = """
        uniform sampler2D g_Texture0;
